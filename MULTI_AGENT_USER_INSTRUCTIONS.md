...
- For long mechanical execution plans (many small steps + "Finished with step X of N, can I continue?"), agents must use *light reporting* in visible responses: narrow forensics only, 1-2 line state appends, and a terse skeleton like "Re-reads (plan + current-state tail + AGENT_* + TODO) done. Git clean at prior builds tag. Step 9 of 12: [one-sentence action per plan]. Pre-forensic: read_file offset/limit on ... Edits applied (N search_replace). Post-forensic: verified [unique string]. git add .kt + current-state + TODO; ./build_app: BUILD SUCCESSFUL. New tag: .... 1-line note appended. Finished with step 9 of 12, can I continue?"
