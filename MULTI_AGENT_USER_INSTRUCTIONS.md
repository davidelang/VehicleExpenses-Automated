...
The approved plan **must** decompose the work into a large number of explicitly named ultra-small micro-phases (typically 8–20+ for non-trivial work). Each phase must consist of the minimal observable edit followed immediately by forensic verification + `git add` (tracked sources + TODO.md) + a **confirmed successful `./build_app`** (new tag recorded) **before** the executor is allowed to start edits for the next phase. The executor must never proceed to phase N+1 until the build gate for phase N has passed. On trouble or partial reset, only the tag of the most recent successful phase (after the mandatory `./get-builds-tag.sh` preflight) may be used. This ultra-micro phased style with per-phase success gates is now the mandatory standard for all execution turns.
...
- On every new planning cycle (when the master tells you to restart the planner terminal with a fresh prompt), include language such as: "Continue strategic planning for the work described in current-state.md (this worktree) and the previous plan at dev-ai-interaction/plans/xxx-plan.md. New feedback: [details]. Produce a revised plan at dev-ai-interaction/plans/yyy-plan.md." (You may also say "prune current-state.md to facts only before updating" if it has grown.)
- The Planning Agent (in its long-lived terminal) and the master are both required to read the local current-state.md first at the start of each new cycle (for branch-specific recent context, decisions, and links) + the designated plan file (in dev-ai-interaction/plans/), then update the local state file while revising the plan document. **current-state.md must hold *facts and pointers only*** — no full user queries, plan text, interaction logs, or re-derivations. See AGENT_MANDATES.md "current-state.md Content and Hygiene Rules" for the exact minimal structure and mandatory prune-before-append ritual (especially during long tiny-step execution phases).
- **Canned hygiene ritual agents must follow before any append to current-state.md** (during execution or after compaction): (1) read current/tail, (2) delete/collapse all completed or "done wrong" items older than the last 3-4 steps (replace mistakes with "Corrected in step Y at tag Z: [1 sentence]"), (3) strip any full quotes of plans/messages, (4) keep total small (target <8KB). Append only the minimal: "Step N: [one sentence]. Tag: XXX. [1 decision if any]." Add "Pruned old facts; see plan + git since LAST_TAG" if a roll was needed.
- For long mechanical execution plans (many small steps + "Finished with step X of N, can I continue?"), the Planning Agent must use *light reporting* in visible responses: narrow forensics only, 1-2 line state appends, and a terse skeleton like "Re-reads (plan + current-state tail + AGENT_* + TODO) done. Git clean at prior builds tag. Step 9 of 12: [one-sentence action per plan]. Pre-forensic: read_file offset/limit on ... Edits applied (N search_replace). Post-forensic: verified [unique string]. git add .kt + TODO; ./build_app: BUILD SUCCESSFUL. New tag: .... (current-state.md updated but never git add'ed). 1-line note appended. Finished with step 9 of 12, can I continue?"
