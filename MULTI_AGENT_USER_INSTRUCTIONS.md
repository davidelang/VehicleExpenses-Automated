...
After you send one of the above, the agent may exit plan mode (if still in it), update TODO.md via `./todo-append` (first execution action; the master ensures wrappers are used for initial TODO and ENGINEERING_LOG entries), re-read the designated plan file, and begin the small decomposed steps.
