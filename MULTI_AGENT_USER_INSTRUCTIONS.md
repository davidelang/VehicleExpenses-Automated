...
- For long mechanical execution plans (many small steps + "Finished with step X of N, can I continue?"), the Planning Agent must use *light reporting* in visible responses: narrow forensics only, 1-2 line state appends, and a terse skeleton like "Re-reads (plan + current-state tail + AGENT_* + TODO) done. Git clean at prior builds tag. Step 4 of 7: [one-sentence action per plan]. Pre-forensic: read_file offset/limit on ... Edits applied (N search_replace). Post-forensic: verified [unique string]. git add .kt + TODO; ./build_app: BUILD SUCCESSFUL. New tag: .... (current-state.md updated but never git add'ed). 1-line note appended. Finished with step 4 of 7, can I continue?"
