...
- **Shell cwd:** Use `working_directory` on Shell when you need a different directory — do **not** prefix blessed helpers (`./append-to-engineering-log`, `./build_app`, `./get-builds-tag.sh`) with `cd path &&` (breaks permission allow patterns and prompts the user). Run `pwd` once if unsure; invoke helpers directly from the worktree root.
