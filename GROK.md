...
- **Shell cwd:** Use `working_directory` on Shell when you need a different directory — do **not** prefix blessed helpers with `cd path &&` unless `path` is inside the `VehicleExpenses-automated` worktree (hook rejects `cd /elsewhere && …`). Prefer `pwd` once, then invoke `./append-to-engineering-log` / `./build_app` directly.
