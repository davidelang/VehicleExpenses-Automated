...
5. **Merge Strategy Proposal:** Your proposed Integration Strategy MUST be exactly this (copy-paste):
    ...
    - Prefer **`./merge-branch-into-master.sh <branch-name>`** from the master worktree (runs `./install-merge-drivers.sh`, tries `git merge --no-autostash`, falls back to index-first +a-safe path for `ENGINEERING_LOG.md`, **`restore_special`** for TODO/facts, prints always-on checklist). Do **not** invent a temporary branch solely because of `chattr +a` — eng-log merges via `ve-englog` + `./append-to-engineering-log` without clearing append-only.
