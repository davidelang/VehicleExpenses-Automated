...
Magic path approval **is** execute — do **not** ask whether to implement.

**Mandatory phased discipline with per-phase gates (non-negotiable):** Follow the approved plan's **Phased Execution** section. Each phase is a coherent unit of work. Per-phase gates (forensic read/grep, successful `./build_app` before next phase) are in the STANDARD BLOCK and Baseball Rule — do not skip them. Do **not** `git add` or `git commit` (only `./build_app`).
