...
- Execution start (after explicit user magic approval that names *this exact sandbox plan path*): re-read this plan + project-facts.md (perform hygiene pass first). Update TODO.md as the very first action.
...
- **Mandatory ultra-micro phased execution with per-phase success gates**: The plan's "Phased Small-Step Execution" section must decompose the work into as many explicitly named, ultra-small phases as needed to keep each phase the smallest practical observable unit of work that can be forensically verified in isolation and completed with a successful `./build_app`. There is no target number of phases — the goal is maximum safe granularity (more smaller phases is better than fewer larger ones, as long as each is a meaningful, independently verifiable and buildable step). Each phase performs the smallest observable edit, followed **immediately** by narrow forensic `read_file` (offset/limit) + targeted grep before/after + a call to `./build_app` (e.g. `./build_app @phase_summary.txt changed-file.kt TODO.md ...` — **do not** perform `git add` or direct gradle/gradlew commands manually; `./build_app` handles staging+commit for the listed files + any supplied message) + a **confirmed successful build + new branch-scoped builds tag** (recorded). Project-facts.md may be updated with newly discovered stable facts (include them when calling `./build_app` if relevant). No edits for the next phase may begin until the current phase's `./build_app` has succeeded. On any failure, trouble, or partial reset, only the tag from the most recent successful phase's `./build_app` (obtained via the mandatory `./get-builds-tag.sh` preflight) may be used.

- When calling `./build_app` for a phase: supply a rich commit message (preferably via `@phase_summary.txt` or similar, containing the phase summary or plan excerpt) and explicitly list the changed tracked source file(s) + TODO.md as arguments. `./build_app` performs the staging + commit. (Stable facts updates to project-facts.md may be included in the call when relevant.)

- project-facts.md updates: only new stable codebase facts or pointers that would remain true for other work on the tree (no plan references, no branch/tag, no execution narrative, no "current effort" details). Always read the full file.
