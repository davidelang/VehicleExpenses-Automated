...
- Execution start (after explicit user magic approval that names *this exact sandbox plan path*): re-read this plan + project-facts.md (perform hygiene pass first). Use `./append-to-engineering-log` to record the execution start note as the very first action.
