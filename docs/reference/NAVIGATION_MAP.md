...
- **Start trip**: Open-only trip segments (Business / Personal / …) as fuel rows with **Trip Type**; camera or manual odo; immediately after Quick Fill-up in the drawer.
...
- **Reports**: Experimental reports hub and child sets including **Trip miles** (`reports_lab/trips`; always in drawer).
...
- **Start trip** → (pop back / drawer; saves trip-start fuel rows in place)
...
- **Reports** → child lab routes / drawer
...
- **Start trip route**: `triptracking` (drawer item immediately after Quick Fill-up)
- **Fill history** (fills list) is under **Reports** catalog, not the drawer. Deep link `fuelhistory` may remain.
