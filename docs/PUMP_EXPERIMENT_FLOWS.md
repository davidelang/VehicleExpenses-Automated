...
| `Set Prod-ink` | product **224+608** | Same ink knobs as P4-ink on product reds (walk once; OCR k=1/2/3; official k=1) | Experiment only. PD / `final` = k=1. Pool `scaleVariants` k=1/2/3 `kind=ink`. `s_per_red` |
...
| `Set Prod-rot-ink` | **product_det** **224+608** | Same oriented det as P4-rot-ink on product; AABB ink walk once; OCR k=1/2/3; official k=1 | `finalKind=ink`. PD / `final` = k=1. Pool `scaleVariants` k=1/2/3 `kind=ink`. `s_per_red` |
