...
Thirteen columns. **P4 / v4** discovery **224+1024** (no 2048). **Prod / G--** discovery **224+608**. Heatmap dump tiers stay 224/608/1024/2048. Experiment **G4** column is gone. **Live QF** (`OcrHarness.extractQuickFillG4CostVol`) = experiment **G4-vjump** recipe: v4 det, **224+1024**, verts **0.0/0.1/0.2/0.5**, L/R jump-retract (no 0.5×H). Keep G--, **G4-vjump**, **7seg-stroke**, **Prod-ink** (product AABB ink), P4-jump, **P4-m65p08**, gx, xycut, P4-rot-jump, **P4-rot-ink**, Prod-jump, Prod-rot, **Prod-rot-ink**. Expand energy `final` / ink `finalKind=ink`. Vert cap and energy `maxFrac` stay **0.40**. **Cap is a grow leash only** — it does **not** swap G verts. Ink/energy columns skip vertical grow (and 7seg 1s pad) on a side when the 1px strip just outside red T/B is already empty; the red is never retracted. No 0–0.50 rot sweep.
...
| `Set 7seg-stroke` | **PP-OCRv4_mobile_det** **224+1024** | Same v4 reds as G4-vjump. Seed-ROI Otsu stroke `s` (vSW H-path; drop CCs ≫ 3s). Fallback `0.08×seedH` also if stroke-count share < **0.30** or max non-span horiz run ≥ **0.50×seedW**. Vert: freeze width, walk to gap ≥ `s` with cap **0.40×red H** per side; **k=1s** pad inside that budget; skip grow/1s on a side if the 1px strip just outside red T/B has no bar. Horz: G4-vjump `jumpRetractHorizontal` (`jumpFrac=0.40`, `retractClearFrac=0.30`, `energyRatio=0.65`, `maxFrac=0.4`). **No** 8s / 2s, **no** G-list | Experiment only. Not QF live. PD = reds + stroke blues. Per-red `s` in assembly |
| `Set Prod-ink` | product **224+608** | Same ink knobs as 7seg-stroke on product reds | Experiment only. PD = reds + stroke blues. `s_per_red` |
| `Set P4-jump (v4 + energy + jump, S OCR)` | **PP-OCRv4_mobile_det** **224+1024** | AABB energy **maxFrac=0.4**; cap stops the walk only | Control. `final` = energy. Jump L/R only. **No extra vert pad** |
| `Set P4-m65p08 (v4 m65 + pad 0.08)` | **PP-OCRv4_mobile_det** **224+1024** | Frozen-width mean \|∇\| **0.65** + pad **0.08** + L/R jump. Cap = leash | Locked m65 recipe. `final` = energy |
| `Set P4-gx (v4 + gx0.55 frozen + jump)` | **PP-OCRv4_mobile_det** **224+1024** | Frozen width; energy = **\|∂I/∂x\|**; stop at **0.55** × seed `gx`; **+0.08×seedH** each tip; L/R jump. Cap = leash | `final` = energy |
| `Set P4-xycut (v4 + xycut-gx frozen + jump)` | **PP-OCRv4_mobile_det** **224+1024** | Frozen width; **XY-cut** on the `gx` row profile; **+0.15×seedH** each tip; L/R jump. Cap = leash | `final` = energy |
| `Set P4-rot-jump (v4 oriented + jump, S OCR)` | **PP-OCRv4_mobile_det** **224+1024** | Oriented **m65** energy (ratio **0.65**, width frozen on the height walk, **+0.08×seedH** each tip) + L/R jump. Cap = leash. **No per-seed vert sweep** | `final` = energy. `scaleVariants` has `energy` (and `energy_or_g` ≡ energy) |
| `Set P4-rot-ink` | **PP-OCRv4_mobile_det** **224+1024** | Same oriented det + `pruneOrientedQuads` as P4-rot-jump (**no deskew**). Then AABB `expand7segFromSeed` + `jumpRetractHorizontal` + `orientedFromAabb`. **No** new ±v 7seg walker | `finalKind=ink`. PD = oriented reds + ink blues. `s_per_red` |
| `Set Prod-jump (product + energy + jump, S OCR)` | **product_det** **224+608** | Same AABB energy; **maxFrac=0.4**; no G-- stitch | Product det A/B vs P4-jump |
| `Set Prod-rot (product oriented + jump, S OCR)` | **product_det** **224+608** | Same oriented m65 energy; cap = leash; no sweep | Product det A/B vs P4-rot |
| `Set Prod-rot-ink` | **product_det** **224+608** | Same oriented det as Prod-rot; AABB ink walk as P4-rot-ink | `finalKind=ink`. `s_per_red` |
...
**P-rot / P4-rot-jump / Prod-rot path:** one `minAreaRect` detect per scale; keep 8-corners. Nested/poke merge is **oriented** (`pruneOrientedQuads`): the keeper keeps its tilt; a smaller box that pokes out moves only the keeper sides that need to cover it, each along its own normal. Same gates as G (contain, 40px poke, similar-overlap, top-N) but never an AABB union. Energy rot: JNI `nativeExpandOriented` (+ L/R jump); empty seed T/B strip skips grow/pad on that tip. Kotlin walk only for energy traces or native-null. Rot-ink: AABB 7seg walk + jump, then `orientedFromAabb`. Warp OCR at each `ocrScales` entry. Does **not** call `runDiscoveryPaddle`.
