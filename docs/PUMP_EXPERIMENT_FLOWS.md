...
**Seed poison (7seg):** Coarse Y Otsu+pepper, then poison CCs (fat `min(h,v)-run > 3×max(v0,4)` or `h-run > 11×max(v0,4)`; if `v0≤4` also `h-run > 0.25×seedW`). Re-peak `v0_clean` on non-poison; each poison CC re-peaks. Agreeing peaks may be ink under local Y (walk `sPx` stays clean); no-peak poison is 0. One look raster: `hasBar` if a 1-run (across region edges) is in `[minRun, glareW]`. After that raster, `dropWide` only too-wide **runs**.
