...
Eighteen columns (10 gray + 8 `*-color`). **P4 / v4** discovery **224+1024** (no 2048). **Prod / G--** discovery **224+608**. Heatmap dump tiers stay 224/608/1024/2048. Experiment **G4** column is gone. **Live QF** (`OcrHarness.extractQuickFillG4CostVol`) = experiment **G4-vjump** recipe: v4 det, **224+1024**, verts **0.0/0.1/0.2/0.5**, L/R jump-retract (no 0.5×H). Keep G--, **G4-vjump** (no color siblings). Gray: **chi2-p4**, **ink-p4**, **ink-prod**, **jump-p4**, **jump-prod**, **rot-ink-p4**, **rot-ink-prod**, **xycut-p4**. Color: same names with `-color` suffix (after p4/prod). HTML `toSortedMap()` is gray then color sibling. Ingest **keeps UV**; Paddle det/rec still **Y** (`populateMonoUInt8` on `mat`). Color expand: fused `hypot(|∇Y|,|∇C|)` for jump/xycut/chi2 vertical walk; ink 7seg on chromaMag (seed median chromaMag &lt; 8 → Y fallback). L/R jump-retract stays on **Y magnitude**. Expand energy `final` / ink `finalKind=ink`. Energy `maxFrac` stays **0.40**. Ink: walk **once** (empty skip **0.5`s`**, start peek **0.5`s`**, safety cap **2.5×red H**, **no k-pad in the walk**); then pad **k=1** (official) or **k=0** (zero vert pad, walk-stop T/B) each tip (k=1 clamp to remaining cap), jump, OCR **k=1 then k=0** (`scaleVariants` `kind=ink`, `s`=k). Official `final` / PD blue = **k=1**. k=2/3 gone. Energy columns still skip vertical grow on a side when the 1px strip just outside red T/B is already empty; the red is never retracted. No 0–0.50 rot sweep. Energy `ocrScales` stay `[1.0]`.
...
| `Set chi2-p4` | **PP-OCRv4_mobile_det** **224+1024** | Frozen seed width; row-gray **χ² vs seed hist** (16 bins, `chi2K=3.5`, consec **2**); cap **0.4×seedH**; first outside row already stop-true → no walk/pad that tip; then **+0.08×seedH** on allowed tips; L/R jump on **Sobel magnitude** (`energyRatio=0.65`, `jumpFrac=0.40`, `retractClearFrac=0.30`). **No** G-list | Experiment only. `final` = expand crop. Jump thr is **0.65**, not 3.5 |
| `Set chi2-p4-color` | **PP-OCRv4_mobile_det** **224+1024** | Same knobs as chi2-p4; vertical χ² on **chromaMag**; fused energy walk helpers; L/R jump on **Y mag** | Color sibling. Rec still Y |
| `Set ink-p4` | **PP-OCRv4_mobile_det** **224+1024** | Same v4 reds as G4-vjump. Seed-ROI Otsu stroke `s` (vSW H-path; drop CCs ≫ 3s). Fallback `0.08×seedH` also if stroke-count share < **0.30** or max non-span horiz run ≥ **0.50×seedW**. Vert: freeze width, walk **once** (k=0 in walk) to gap ≥ **0.5`s`**; start peek **0.5`s`** outside red (freeze that side only if the peek is empty); cap **2.5×red H** per side (**safety**). Then `padVertByStrokes` **k=1** official / **k=0** test (zero vert pad); Horz jump after **each** pad (`jumpFrac=0.40`, `retractClearFrac=0.30`, `energyRatio=0.65`, `maxFrac=0.4`). OCR k=1 then k=0. **No** 8s / 2s, **no** G-list | Experiment only. Not QF live. PD / `final` = k=1 blues. Pool `scaleVariants` k=1 then k=0 `kind=ink`. Per-red `s` in assembly |
| `Set ink-p4-color` | **PP-OCRv4_mobile_det** **224+1024** | Same knobs; walk on **chromaMag** (`expand7segFromSeedChroma`); median chromaMag &lt; **8** → Y `expand7segFromSeed`. Jump on Y | Color sibling. Rec still Y |
| `Set ink-prod` | product **224+608** | Same ink knobs as ink-p4 on product reds (walk once; OCR k=0/1; official k=1) | Experiment only. PD / `final` = k=1. Pool `scaleVariants` k=1 then k=0 `kind=ink`. `s_per_red` |
| `Set ink-prod-color` | product **224+608** | Same as ink-p4-color on product reds | Color sibling |
...
| `Set jump-p4-color` | **PP-OCRv4_mobile_det** **224+1024** | Vertical walk on fused `hypot(\|∇Y\|,\|∇C\|)`; L/R jump on **Y mag** 0.65; maxFrac 0.4 | Color sibling. Rec still Y |
| `Set jump-prod` | **product_det** **224+608** | Same AABB energy; **maxFrac=0.4**; no G-- stitch | Product det A/B vs jump-p4 |
| `Set jump-prod-color` | **product_det** **224+608** | Same fused energy as jump-p4-color on product | Color sibling |
| `Set rot-ink-p4` | **PP-OCRv4_mobile_det** **224+1024** | Oriented det + `pruneOrientedQuads` (**no deskew**). Then AABB `expand7segFromSeed` **once** (gap/peek **0.5`s`**, cap **2.5×** safety, k=0 in walk) + `padVertByStrokes` k=1 then k=0 + `jumpRetractHorizontal` + `orientedFromAabb`. OCR k=1 then k=0. **No** new ±v 7seg walker | `finalKind=ink`. PD / `final` = k=1. Pool `scaleVariants` k=1 then k=0 `kind=ink`. `s_per_red` |
| `Set rot-ink-p4-color` | **PP-OCRv4_mobile_det** **224+1024** | Same oriented det; AABB `expand7segFromSeedChroma` + k=1 then k=0 + Y jump | Color sibling |
| `Set rot-ink-prod` | **product_det** **224+608** | Same oriented det as rot-ink-p4 on product; AABB ink walk once; OCR k=0/1; official k=1 | `finalKind=ink`. PD / `final` = k=1. Pool `scaleVariants` k=1 then k=0 `kind=ink`. `s_per_red` |
| `Set rot-ink-prod-color` | **product_det** **224+608** | Same as rot-ink-p4-color on product | Color sibling |
...
| `Set xycut-p4-color` | **PP-OCRv4_mobile_det** **224+1024** | XY-cut on fused \|∂Y/∂x\|+\|∂C/∂x\|; +0.15 pad; L/R jump on Y mag | Color sibling |
...
**rot-ink-p4 / rot-ink-prod path:** one `minAreaRect` detect per scale; keep 8-corners. Nested/poke merge is **oriented** (`pruneOrientedQuads`): the keeper keeps its tilt; a smaller box that pokes out moves only the keeper sides that need to cover it, each along its own normal. Same gates as G (contain, 40px poke, similar-overlap, top-N) but never an AABB union. Rot-ink: AABB 7seg walk **once** (k=0 in walk), pad k=1 then k=0, jump, then `orientedFromAabb`; OCR `kind=ink` at k=1 then k=0 (`final` = k=1). JNI `nativeExpandOriented` (one-sided T/B skip) remains for parked energy-rot helpers. Energy rot (parked) still warps OCR at each `ocrScales` entry. Does **not** call `runDiscoveryPaddle`.
...
**UI subset button:** **Horiz-affected (76)** still filters photos; columns are the full active set above. Deep link: `vehicleexpenses://experiment/pump?auto=horiz`. **Prod-ink fail (60)** filters to photos where ink-prod or rot-ink-prod is not exact (relax=fail) on 23-18 phone+tablet **and** some other scheduled column exacts that field (`union_salvable`); columns still the full scheduled set (18). Deep link: `vehicleexpenses://experiment/pump?auto=prodinkfail`. **Det dump (P4/prod × deskew/rot)** is a one-shot discover button (not every 18-col run): four recipes, no expand/OCR, all `pump_photos`, sidecar `pump_reports/pump_det_boxes_<ts>/`. Deep link: `vehicleexpenses://experiment/pump?auto=detdump`. Fetch pulls the newest `pump_det_boxes_<ts>/` even without a matching `pump_results`.
