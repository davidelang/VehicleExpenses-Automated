...
| `Set P4-rot-ink` | **PP-OCRv4_mobile_det** **224+1024** | Oriented det + `pruneOrientedQuads` (**no deskew**). Then AABB `expand7segFromSeed` + `jumpRetractHorizontal` + `orientedFromAabb`. **No** new ±v 7seg walker | `finalKind=ink`. PD = oriented reds + ink blues. `s_per_red` |
