...
**JNI content expand (AABB):** Gray and color AABB energy (jump / chi2 / xycut), ink 7seg, and Y jump-retract are JNI (`nativeAabbGrowMany` / `nativeSeg7Many` / `nativeJumpMany` in `ContentExpandNative.cpp`). One Sobel set (and chromaMag on color) per photo for all seeds. `nativeSeg7Many` `chromaMode`: 0 gray, 1 chromaMag, 2 **tintMask** (`fillChromaTintMask`, 3× glare), 3 same tintMask with **11×** glare. Scheduled **rot-ink** 7seg/jump run in JNI (`nativeSeg7OrientedMany` / `nativeJumpOrientedMany`); Kotlin `expand7segFromOrientedSeedMany` / `jumpRetractOrientedUMany` fallback if native returns null; rec warps the finished blue. Parked rot-energy still uses `nativeExpandOriented`. G-- verts stay calculated. Rec still Y. Kotlin bodies are fallback if JNI returns null, and when energy traces are on.
