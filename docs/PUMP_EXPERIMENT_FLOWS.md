...
**HTML look ink:** gray/color 7seg (AABB and rot) dump the **combined** 255 look plane (white=ink) from **B.s** (full frame; **A.p** stays the photo). Per-seed JPEG of seed + look strip, same 48px-high 1:1 wrap as rec, label `boxN`. Red seed T/B; **yellow** full-width at the landed ink row when that side used its one-shot gap jump. Sibling **Look ink** checkbox (class `look-ink-crops`). Official walk / rec feed unchanged.
