...
| `Set rot-ink-prod` | **product_det** **224+608** | Oriented det + `pruneOrientedQuads` (**no deskew**). AABB 7seg walk once + pad k=1 official / k=0,2,3,4 unofficial + Y jump + `orientedFromAabb`. OCR **k=0..4** | `finalKind=ink`. PD / `final` = k=1 |
...
**rot-ink-prod path:** one `minAreaRect` detect per scale; keep 8-corners. Nested/poke merge is **oriented** (`pruneOrientedQuads`). Rot-ink: AABB 7seg walk **once** (k=0 in walk), pad k=1 official / k=0,2,3,4 unofficial, jump, then `orientedFromAabb`; OCR `kind=ink` **k=0..4** (`final` = k=1). JNI `nativeExpandOriented` remains for parked energy-rot helpers. Does **not** call `runDiscoveryPaddle`.
