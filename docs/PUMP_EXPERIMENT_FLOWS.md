...
| `Set rot-ink-prod-color` | **product_det** **224+608** | Same oriented det; chromaMag 7seg on `±v` strips + Y jump along `±u`. Rec warps finished blue | Color sibling |
...
**rot-ink-prod path:** one `minAreaRect` detect per scale; keep 8-corners. Nested/poke merge is **oriented** (`pruneOrientedQuads`). Rot-ink: 7seg walk **along red normals** in the original image (`±v`, freeze `u` span; peek/gap/cap same as AABB ink). Then k-pad along `v`, jump-retract along `±u`. **Do not** `toAabb` before that walk. PD blues are that parallelogram. Rec warps the **finished** blue only (`inflate` 4px, `INTER_CUBIC`). OCR `kind=ink` **k=0..4** (`final` = k=1). JNI `nativeExpandOriented` remains for parked energy-rot helpers. Does **not** call `runDiscoveryPaddle`.
