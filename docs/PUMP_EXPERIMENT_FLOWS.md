...
| `Set G4-vjump (v4 + verts 0–2.5/0.1 + L/R jump)` | **PP-OCRv4_mobile_det** | Calculated height pads **0.0 … 2.5 step 0.1** (26, both tips); width = P4-jump L/R jump-retract (`jumpFrac=0.40`, `retractClearFrac=0.30`, `energyRatio=0.65`, `maxFrac=0.4`). **No** 0.5×H, **no** energy vertical walk | Experiment only. Not QF live. PD = reds + jumped blues (no orange) |
