...
Nine columns. **P4 / v4** discovery **224+1024** (no 2048). **Prod / G--** discovery **224+608**. Heatmap dump tiers stay 224/608/1024/2048. Experiment **G4** column is gone. **Live QF** (`OcrHarness.extractQuickFillG4CostVol`) = experiment **G4-vjump** recipe: v4 det, **224+1024**, verts **0.0/0.1/0.2/0.5**, L/R jump-retract (no 0.5×H). Keep G--, **G4-vjump**, **P4-ink** (v4 AABB ink; was 7seg-stroke), **Prod-ink**, P4-jump, xycut, **P4-rot-ink**, Prod-jump, **Prod-rot-ink**. Expand energy `final` / ink `finalKind=ink`. Energy `maxFrac` stays **0.40**. Ink walk: empty skip **0.5`s`**, start peek **0.5`s`**, safety cap **2.5×red H**, always **k=1s** pad. Energy columns still skip vertical grow on a side when the 1px strip just outside red T/B is already empty; the red is never retracted. No 0–0.50 rot sweep.
...
| `Set P4-ink` | **PP-OCRv4_mobile_det** **224+1024** | Same v4 reds as G4-vjump. Seed-ROI Otsu stroke `s` (vSW H-path; drop CCs ≫ 3s). Fallback `0.08×seedH` also if stroke-count share < **0.30** or max non-span horiz run ≥ **0.50×seedW**. Vert: freeze width, walk to gap ≥ **0.5`s`**; start peek **0.5`s`** outside red (freeze that side only if the peek is empty); cap **2.5×red H** per side (**safety**); **always k=1s** pad clamped to remaining cap. Horz: G4-vjump `jumpRetractHorizontal` (`jumpFrac=0.40`, `retractClearFrac=0.30`, `energyRatio=0.65`, `maxFrac=0.4`). **No** 8s / 2s, **no** G-list | Experiment only. Not QF live. PD = reds + stroke blues. Per-red `s` in assembly |
| `Set Prod-ink` | product **224+608** | Same ink knobs as P4-ink on product reds | Experiment only. PD = reds + stroke blues. `s_per_red` |
...
| `Set P4-rot-ink` | **PP-OCRv4_mobile_det** **224+1024** | Oriented det + `pruneOrientedQuads` (**no deskew**). Then AABB `expand7segFromSeed` (gap/peek **0.5`s`**, cap **2.5×** safety, always k=1s) + `jumpRetractHorizontal` + `orientedFromAabb`. **No** new ±v 7seg walker | `finalKind=ink`. PD = oriented reds + ink blues. `s_per_red` |
...
| `Set Prod-rot-ink` | **product_det** **224+608** | Same oriented det as P4-rot-ink on product; AABB ink walk | `finalKind=ink`. `s_per_red` |
...
**P4-rot-ink / Prod-rot-ink path:** one `minAreaRect` detect per scale; keep 8-corners. Nested/poke merge is **oriented** (`pruneOrientedQuads`): the keeper keeps its tilt; a smaller box that pokes out moves only the keeper sides that need to cover it, each along its own normal. Same gates as G (contain, 40px poke, similar-overlap, top-N) but never an AABB union. Rot-ink: AABB 7seg walk + jump, then `orientedFromAabb`. JNI `nativeExpandOriented` (one-sided T/B skip) remains for parked energy-rot helpers. Warp OCR at each `ocrScales` entry. Does **not** call `runDiscoveryPaddle`.
