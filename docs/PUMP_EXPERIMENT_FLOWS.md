...
Each column = fresh master. H/L/M/N/O/Q parked — `docs/obsolete/EXPERIMENT_PUMP_SETS.md`. Device HTML is `pump_report_<ts>_partNN.html` split by photo count (default 50; 0 = one `_part01` with the whole run list). Always `_partNN`. Toolbar Prev/Next part are relative links (disabled at ends). In-page column / orig / dump / rec-crop / look-ink / Rows filters and prev/next photo unchanged.

**13 columns:** `Set G--` (live QF), 2 energy AABB, 2 color AABB, 2 energy rot, 2 color rot, **4 color-vsp**. Gray AABB/rot tight/retract are **unscheduled** (processors parked). The six `*-base` columns are **unscheduled**. **Prod / G--** discovery **224+608**. v4/p4 dropped (`docs/obsolete/DROP_P4_P5_DET.md`). **Live QF** = G--: product det, **224+608**, verts **0.1/0.3/0.4/1.1**, horiz **0.5**. Scheduled `walk2`, legacy `color`, and `color2` are **parked**. Masks: **energy** = INTERIOR_ENERGY expand (jump-prod-like, `maxFrac=0.4`); **gray** = 7seg Y Otsu (parked); **color** = 7seg `color_adaptive` (`chromaMode=4`: meanChromaInk&lt;12 **or** `uInk·uBg≥0.50` → walk gray Y; else tint with panel-hue veto — chromatic panel (cos≥0.50 vs uBg) is not ink; Y-near or ink-hue can be ink without matching stroke hue); **vsp** = same color path with **virtual** salt-and-pepper (SW/gap skip-in-run, **no** look-bin fill). Bound: **tight** = inside pixel, expands; **edge-retract** 1px expand if edge has ink else retract inward (50% cap, no outward jump). No 16px inset. Tight+retract vertical walk: consecutive `hasBar` then **one** gap jump ≤ `0.5×sPx` empty if a `hasBar` appears (at most one per side; empty-edge peek **is** that jump). Jump once then pad k. Ingest **keeps UV**; rec still **Y**. Color (non-vsp) look salt-pepper **H only**. 255-flood classify then recover; poison (H-run **>6×sPx** / V-run **>16×sPx** only; no 3×∧2× fat) **before** one `walkV`. GlareW **6×sPx**. L/R jump **iterative 1px** (`max_jumps=4`). Color official `final` = **unexpanded k=0**; PD draws **both** k=0 blues (walk + 0.6H pad). Extras k=1..4; skip extra-k as live from unexpanded k=0 asis letters-no-digits. Rec strip **40+4+4=48**. Energy `ocrScales` `[1.0]`. Dump-details **inkSweep**: per-seed 1-D V/H ink scores vs offset (same function as live expand/jump). 7seg sweep/hists are look-id (`isLookInkId`) in the look window. Hidden with Dump details: two SVG sparklines per box (score, thr line, seed ticks, walk/jump ticks). Color `minRun` is per-seed (`min(0.5s, 0.4×max in-seed run)`); edge-retract cap is **50% seedH** (`BLOCKED_RETRACT_LIMIT`); 7seg AABB H jump is **0.60×walked H**, max 4, miss + retract-found-ink continues (energy/G-- stay **0.40×H**). G-- has no `inkSweep`. Rot profiles step ±v/±u.
...
| `Set ink-color-tight` | product **224+608** | `color_adaptive` 7seg (`chromaMode=4`); fill-S&P; inset **16px** | metadata `color_adaptive` |
...
**Parked** (source, not scheduled): gray AABB/rot tight/retract; prior ink-prod / color / color2 / walk2 / jump-prod / rot-ink-prod, P / P-jump / P-rot / Prod-m65, H* horiz, L/M dilate, G-dense, K. v4/p4 processors deleted (`obsolete-p4-p5-det`).
...
**JNI content expand:** No flag/`*Many` JNI (`nativeAabbGrowMany` / `nativeSeg7Many` / `nativeJumpMany` / `nativeJumpOrientedMany` / `nativeExpandOriented` deleted). Scheduled First 10: energy Tight/Retract, color Tight/Retract (fill-S&P), color TightVsp/RetractVsp (virtual S&P). Gray Tight/Retract natives remain compiled but **unscheduled**. Parked 7seg/energy (ink-prod / color / walk2 / rot-ink-prod / jump-prod-color / `*-base`) use Expand recipes (`nativeEnergyAabbExpand` / `nativeEnergyOrientExpand` / `nativeGrayAabbExpand` / `nativeColorAabbExpand` / `nativeGrayOrientExpand` / `nativeColorOrientExpand`); still unscheduled. Gray/color orient Tight/Retract/Expand remain (static C++ `seg7OrientedMany`). G-- calculated verts. Native fail = seed. Energy traces skip JNI (`recordVertEnergy` → seed). Rec still Y. Color look salt-pepper **H only**; vsp does **not** rewrite the bin. 255-flood **pix-set runs** + `lookAlloc` (253-- / 1++); poison **only** H-run >6×sPx or V-run >16×sPx (no 3×∧2× fat); recover ink-adjacent in **walk-edge Y bands** (first seed±4×sPx, overlap ≥sPx, after H jump); `hasBar`/`kind[]` ink (254 or `kKindInk`); one `walkV`; glareW **6×sPx**. No `dropWide`/`dropTallCCs`/`dropWideRuns`. look_ink `ccs` = those flood-poison runs. Color skipTint (`meanChromaInk<12` **or** `uInk·uBg≥0.50`) stays inside color Expand/Tight/Retract.

**7seg Y Otsu fill (red/seed only, not energy):** `(look-bin + 8-connected recovered poison ink) / seed area`; recovered is **in** look-bin (`hasBar`); walk/`sPx` from the **final** seed; band **0.05–0.45**. Ceiling (`fill>0.45`) less-ink extras (`retryWhy=2`, `nRetry` = extras). Floor extras (`retryWhy=1`, cap 3) if `fill<0.05` **or** `nLookBin==0`. In-band SW-discard remaining valleys (`retryWhy=3`, first in-band **and** accepted sPx). look_ink `attempts[]` has per-evaluate thr/fill/keep/poison (energy has none).
...
| **Reports** | `files/multi_scale_det_reports/` **flat** — `multi_scale_det_report_<ts>_partNN.html` (photo-count chunks, default 50; 0 = one `_part01`), `results_<ts>.sparse`, `status/manifest/cursor_<ts>.*` filled by collapser (1 min ticks); tray only: `cells/` (pre-merge fragments, purged). JSON/sparse/cursor unsplit. No `run_*` subdirs. Fetch via `fetch_latest_reports.py`. |
