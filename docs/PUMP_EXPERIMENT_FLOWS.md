...
**7seg Y Otsu fill (red/seed only, not energy):** `(look-bin + 8-connected recovered poison ink) / seed area`; recovered is **in** look-bin (`hasBar`); walk/`sPx` from the **final** seed; band **0.05–0.45**. Ceiling (`fill>0.45`) is observe-only (`retryWhy=2`, `nRetry=0`). Extras are **floor-only** (cap 3); commit if in-band or attempt-0 empty look-bin becomes nonzero. look_ink `attempts[]` has per-evaluate thr/fill/keep/poison (energy has none).
