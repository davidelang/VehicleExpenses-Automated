...
**7seg Y Otsu fill (red/seed only, not energy):** `(look-bin + 8-connected recovered poison ink) / seed area`; recovered is **in** look-bin (`hasBar`); walk/`sPx` from the **final** seed; band **0.05–0.45**; at most **3** extras (keep-set valleys; empty keep-set floor may rebuild poison once). look_ink `attempts[]` has per-evaluate thr/fill/keep/poison (energy has none).
