...
- Pump G-family vert lists: `ui/util/PumpCostVolUtils.kt` — `SET_G_VERT_FACTORS` (6), `SET_G_MINUS_VERT_FACTORS` (3), `SET_G_MINUS_MINUS_VERT_FACTORS` (2); Quick Fill uses G-- via `OcrHarness.kt`
- Hybrid stretch helpers: `ui/util/OdometerOcrUtils.kt` — `getClipStretchLowHigh`, `getValleyPeakGrays`, `applyValleyPushWithGrays` (used by ExperimentPumpScreen procH/procI)
