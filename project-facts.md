...
- Git LFS: unscheduled Paddle **server** dets (`third_party/paddle/exp_det_ab_unscheduled/PP-OCR*_server_det_*.nb`) — never APK; GitHub 100 MiB hook. Need `git-lfs` on PATH (`sudo apt install git-lfs`) and `git lfs pull`. Pointers in `.gitattributes`. See `docs/obsolete/EXPERIMENT_DET_MODELS.md`.
