...
- Pump rot 7-seg walk is u/v math on the photo plane (`A.p` gray; tint `A.s` at photo size). Look-bin is an A.s/B.s view ≤ photo. Overlay paint is inside seed u/v. Warp is look-ink JPEG + rec only (not a uv search canvas).
