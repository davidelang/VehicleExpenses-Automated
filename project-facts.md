...
- Pump rot 7-seg walk is u/v math on the photo plane (`A.p` gray; tint `A.s` at photo size). Look-bin is an A.s/B.s view ≤ photo. Overlay paint is inside seed u/v. White = walk ink (on and off seed); purple = too-wide/too-tall only. Warp is look-ink JPEG + rec only (not a uv search canvas). Rot look-ink dest is the k-pad union scaled so seed short-axis is 96.
...
- **Deploy:** APK-first installs the matching ABI flavor APK from `ro.product.cpu.abi` (`app/build/outputs/apk/<arm64|x86_64|armv7>/debug/app-<flavor>-debug.apk`). `--rebuild` is per-device `installArm64Debug` / `installX86_64Debug` / `installArmv7Debug`. Version from APK `versionName` when not rebuilding. Human-only (agents do not run `./deploy` / `adb install`). After merging stacked wrappers, human publishes orch via `./publish-stacked-wrappers-to-orch --apply` from `master/`, then orch `./update-rules.sh` (no `--force`).
