...
- Scripts: `update-rules.sh`, `build_app` and `deploy` (no raw gradlew; both pass `--no-daemon`; deploy wipes kspCaches before compile), `get-builds-tag.sh`, `fix-perms` (rare), `setup_agent.sh` — prefer `source ./setup_agent.sh <branch>` so the shell ends in the new worktree (`./setup_agent.sh` cannot change parent cwd). `project.config` is gitignored; setup seeds it from orchestration before checkout. Built setuid binaries `run-as-primary` / `ve-refresh-shell` are gitignored (`.c` is tracked). `remove_worktree.sh`, `generate_pr.sh`, `cleanup_pr.sh`
