...
- **Permission blocks (policy):** If an agent hits a Unix/Landlock/sudo permission denial it cannot clear with existing allowed tools, **stop and report to the human** — do **not** invent workarounds (chmod games, alternate paths, variable indirection, running as a different user via creative shells, etc.). Human will fix perms or ask for a script they run. Prefer residual + exact command over “creative” bypass.
