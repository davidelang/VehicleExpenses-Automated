...
read_grok_token() {
    local auth_file="$HOME/git/grok/auth.json"
    local scope="$1"
...
DOWNLOAD_DIR="$HOME/git/grok/downloads"
BIN_DIR="${GROK_BIN_DIR:-$HOME/git/grok/bin}"
...
mkdir -p "$HOME/git/grok/completions/bash" "$HOME/git/grok/completions/zsh"
"$BIN_DIR/grok" completions bash > "$HOME/git/grok/completions/bash/grok.bash" 2>/dev/null || true
"$BIN_DIR/grok" completions zsh  > "$HOME/git/grok/completions/zsh/_grok"     2>/dev/null || true
...
CONFIG_FILE="$HOME/git/grok/config.toml"
...
if [ -n "$GROK_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/git/grok/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/git/grok/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/git/grok/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/git/grok/requirements.toml"
        fi
    fi
fi
...
fish_add_path $HOME/git/grok/bin
# <<< grok installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> grok installer >>>
export PATH="$HOME/git/grok/bin:$PATH"
...
# <<< grok installer <<<'
        ...
        new_block='# >>> grok installer >>>
export PATH="$HOME/git/grok/bin:$PATH"
[[ -r "$HOME/git/grok/completions/bash/grok.bash" ]] && source "$HOME/git/grok/completions/bash/grok.bash"
# <<< grok installer <<<'
    fi
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    ...
    echo '  export PATH="$HOME/git/grok/bin:$PATH"' >&2
fi
