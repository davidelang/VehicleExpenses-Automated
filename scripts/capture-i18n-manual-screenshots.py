...
Usage:
  python3 scripts/capture-i18n-manual-screenshots.py [--serial emulator-5556] [--locales es,fr] [--phase pilot|all|west|rest|en] [--strict]
"""
...
LOCALE_BCP47 = {
    ...
    "pl": "pl",
    "ru": "ru",
    # "in" is the legacy ISO code that Android resource matching uses for Indonesian
    # (pref tag remains product "id"; AppLanguage.bcp47 is "in").
    "id": "in",
    "vi": "vi",
    "tr": "tr",
}
...
def focused_package(serial: str) -> str:
    out = sh(serial, "dumpsys window | grep -E 'mCurrentFocus|mFocusedApp' | head -5", check=False)
    # e.g. mCurrentFocus=Window{... com.davidlang.vehicleexpensesautomated/....}
    m = re.search(r"([a-zA-Z0-9_.]+)/[a-zA-Z0-9_.]+", out)
    if m:
        return m.group(1)
    m2 = re.search(r"package=([a-zA-Z0-9_.]+)", out)
    return m2.group(1) if m2 else ""


def assert_app_focused(serial: str, *, strict: bool) -> None:
    pkg = focused_package(serial)
    if pkg != PKG:
        msg = f"focused package is {pkg!r}, expected {PKG} (launcher/other?)"
        if strict:
...
def prove_locale_nav(serial: str, tag: str, s: dict[str, str], *, strict: bool) -> None:
    """After language set, prove a localized nav label is visible (not English for non-en)."""
    open_drawer(serial, s)
    time.sleep(0.4)
    blob = ui_all_text(serial)
    press_back(serial)
    time.sleep(0.3)
    # Prefer Settings / Quick Fill / Help as high-confidence chrome
    candidates = [
        s.get("nav_settings", ""),
        s.get("nav_quick_fill", ""),
        s.get("nav_help", ""),
        s.get("nav_about", ""),
        s.get("nav_reports", ""),
    ]
    ...
    for lab in candidates:
        if lab and lab in blob:
            found_local = True
            # For non-en, reject pure English chrome when translation differs
            if tag != "en":
                # map key by reverse lookup of value in s
                pass
            break
    if tag == "en":
        if not found_local and "Settings" not in blob and "Quick" not in blob:
            msg = "EN nav labels not visible after language set"
            if strict:
                raise RuntimeError(msg)
            print(f"  WARN: {msg}", flush=True)
        return
    # Non-English: require at least one string that is not the EN value
    localized_hits = []
    for key in ("nav_settings", "nav_quick_fill", "nav_help", "nav_about", "nav_reports", "nav_manage_vehicles"):
        loc_lab = s.get(key, "")
        en_lab = en.get(key, "")
        if loc_lab and loc_lab in blob and loc_lab != en_lab:
            localized_hits.append(loc_lab)
    if not localized_hits:
        # Indonesian / others may still show partial EN if pack missing; fail strict
        msg = (
            f"locale {tag}: no non-English nav label found in drawer UI "
            f"(sample: {blob[:200]!r}…)"
        )
        if strict:
            raise RuntimeError(msg)
        ...
        print(f"  locale nav OK: {localized_hits[0]!r}", flush=True)
...
def capture(serial: str, out: Path, *, strict: bool = False, prev_md5: str | None = None) -> str:
    """Capture screenshot; return md5. In strict mode reject launcher; retry consecutive duplicate md5."""
    out.parent.mkdir(parents=True, exist_ok=True)
    helper = ROOT / "scripts/capture-manual-shot.sh"
    last_md5 = ""
    attempts = 3 if strict else 1
    for attempt in range(attempts):
        assert_app_focused(serial, strict=strict)
        ...
        md5 = file_md5(out)
        last_md5 = md5
        if strict and prev_md5 and md5 == prev_md5 and attempt < attempts - 1:
            print(f"  retry duplicate MD5 {out.name}", flush=True)
            # nudge UI slightly
            swipe(serial, 672, 1800, 672, 1600, 200)
            time.sleep(0.4)
            continue
        if strict and prev_md5 and md5 == prev_md5:
            print(f"  WARN: consecutive duplicate MD5 kept for {out.name}", flush=True)
        return md5
    if strict and (not out.is_file() or out.stat().st_size < 1000):
        raise RuntimeError(f"capture too small/missing: {out}")
    return last_md5 or (file_md5(out) if out.is_file() else "")


def navigate_and_capture(serial: str, tag: str, out_dir: Path, *, strict: bool = False) -> dict[str, str]:
    """Return notes per missing/fallback shot."""
    notes: dict[str, str] = {}
    s = load_strings(tag)
    print(f"=== locale {tag} → {out_dir} strict={strict}", flush=True)
    set_language(serial, tag)
    assert_app_focused(serial, strict=strict)
    prove_locale_nav(serial, tag, s, strict=strict)
    dismiss_onboarding(serial, s)
    go_quick_fill(serial, s)
    prev_md5: str | None = None

    def do_capture(path: Path) -> None:
        nonlocal prev_md5
        prev_md5 = capture(serial, path, strict=strict, prev_md5=prev_md5)
    ...
    # --- r3 quick fill live ---
    go_quick_fill(serial, s)
    do_capture(shot("r3-quickfill-odo-live.jpg"))
    ...
    tap(serial, 672, 2400)
    time.sleep(1.5)
    do_capture(shot("r4-quickfill-odo-result.jpg"))
    ...
    tap(serial, 900, 2400)
    time.sleep(0.8)
    do_capture(shot("r5-quickfill-pump-result.jpg"))
    ...
    time.sleep(1.0)
    do_capture(shot("r1-manage-vehicles-crops.jpg"))
    ...
    time.sleep(0.8)
    do_capture(shot("03b-manage-vehicles-new.jpg"))
    press_back(serial)
    ...
    for n in root.iter():
        ...
        if t and t not in ("",) and any(k in t for k in ("Honda", "Ford", "Van", "vehicle", "Vehicle")):
            ...
            if c and c[1] > 400:
                ...
                break
    time.sleep(1.0)
    do_capture(shot("r2-manage-vehicles-landmarks.jpg"))
    if not tapped:
    ...
    time.sleep(1.0)
    do_capture(shot("r6-new-expense.jpg"))
    press_back(serial)
    ...
    time.sleep(1.0)
    do_capture(shot("06-reports.jpg"))
    ...
    time.sleep(0.8)
    do_capture(shot("05-expense-list.jpg"))
    ...
    for n in root.iter():
        ...
        if t and len(t) > 3 and "Expense" not in t and n.attrib.get("clickable") == "true":
            ...
            if c and 500 < c[1] < 2500:
                ...
                break
    time.sleep(0.8)
    do_capture(shot("expense-edit.jpg"))
    if not opened:
    ...
    time.sleep(1.0)
    do_capture(shot("time-based-reports.jpg"))
    swipe(serial, 672, 2200, 672, 800)
    do_capture(shot("time-based-reports-scrolled.jpg"))
    press_back(serial)
    ...
    time.sleep(0.8)
    do_capture(shot("fill-history.jpg"))
    ...
    time.sleep(0.8)
    do_capture(shot("trip-miles.jpg"))
    press_back(serial)
    ...
    time.sleep(1.0)
    do_capture(shot("start-trip.jpg"))
    press_back(serial)
    ...
    time.sleep(1.0)
    do_capture(shot("syncing-hub.jpg"))
    ...
    time.sleep(1.0)
    do_capture(shot("08-spreadsheet-sync.jpg"))
    ...
    if not click_text(serial, add_ss, partial=True, required=False):
        click_text(serial, "Add", partial=True, required=False)
    time.sleep(0.8)
    do_capture(shot("09-spreadsheet-provider-picker.jpg"))
    # pick Google if shown
    if not click_text(serial, "Google", partial=True, required=False):
        press_back(serial)
    else:
        time.sleep(1.0)
        do_capture(shot("10-spreadsheet-google-form.jpg"))
        press_back(serial)
    ...
    time.sleep(1.0)
    do_capture(shot("11-photo-backup.jpg"))
    ...
    time.sleep(1.0)
    do_capture(shot("07-settings.jpg"))
    swipe(serial, 672, 2200, 672, 700)
    do_capture(shot("07b-settings-more.jpg"))
    press_back(serial)
    ...
    time.sleep(1.0)
    do_capture(shot("14-help.jpg"))
    press_back(serial)

    open_drawer(serial, s)
    click_text(serial, s.get("nav_about", "About"), partial=True, required=False)
    time.sleep(1.0)
    do_capture(shot("15-about.jpg"))
    press_back(serial)

    # ensure all 28 exist — fill from previous EN only when not strict
    ...
    if strict and missing:
        raise RuntimeError(
            f"strict: missing/small screenshots for {tag}: {', '.join(missing)} "
            "(no FALLBACK_EN_COPY)"
        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--strict",
        action="store_true",
        help="Reject launcher focus, require localized nav, no EN fallback, retry duplicate MD5",
    )
    args = ap.parse_args()
    serial = args.serial
    # id locale defaults to strict quality bar when not overridden
    strict = args.strict
    ...
    for tag in locales:
        ...
        notes = navigate_and_capture(serial, tag, out_dir, strict=strict)
        all_notes[tag] = notes
        ...
        print(f"  unique_hashes {unique} of {len(hashes)}", flush=True)
        if strict:
            if tag != "en" and same != 0:
                raise RuntimeError(f"strict: identical_to_en={same} for {tag} (want 0)")
            if unique < 20:
                raise RuntimeError(f"strict: unique_hashes={unique} for {tag} (want ≥20)")
        elif tag != "en" and same > 8:
