...
## 2026-08-04 - paddle armv7 tailor + x86 slim (NDK r28c)

- arm64 tailor bit-identical prior pin (~1.8 MB strip)
- armv7 NEW model-tailor (fp32 calib, no FP16): jni/light ~0.75 MB vs prior slim ~3.0 MB; stamps PASS; LINK_OK
- x86 slim thin-jni+light (~10 MB light); android-x86 LITE_BUILD_TAILOR drops KernelRegistrar — default slim
- Emulator matrix doc: docs/reference/PADDLE_ABI_EMULATOR_TEST.md
- Branch libpin-paddle-cleanup; eng-log worktree was truncated under +a — this entry restored via blob commit
