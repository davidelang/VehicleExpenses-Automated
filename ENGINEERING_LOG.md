...
## 2026-08-04 - PR-email-connection prepared after history cleanup

- backup-email-connection @ 45c85772 (pre-rewrite tip); cleaned history → 6 commits, tip 0f6fc574 (same tree as backup).
- Plan archived: historical-plans/email-connection-libpin-pins-paddle-20260803-plan.md
- PR: dev-ai-interaction/PRs/PR-email-connection.md — ready for Master review/merge.
- Did not rebase onto master (rules/pack/gitignore on master left for Master merge).


## 2026-08-04 - libpin-paddle-cleanup start (non-git sources + paddle-models)

- Branch: libpin-paddle-cleanup @ master merge tip fef79693 (no SSH)
- fetch-deps: [[source]] materialize (https/http/file/seed + required sha256; git@/ssh rejected always)
- third_party/paddle-models: file pin for armv8 tailor .nb + .tailored_* (seed + hashes verified)
- paddle/build: prefer paddle-models/src for tailor; armv7 profile hooks; PADDLE_STRIP_MODE; PADDLE_NDK_VERSION Docker ARG; link-gate for armeabi-v7a when present
- Docs: third_party/README.md, paddle SOURCE.md/libpin.toml, paddle-models/SOURCE.md
- Not yet: full armv7 product rebuild, NDK28 image bake/validate, app abiFilters cleanup

## 2026-08-04 - paddle NDK r28c three-ABI verified (libpin-paddle-cleanup)

- Full rebuild arm64-v8a + armeabi-v7a + x86_64 with patches-int8 under NDK r28c (ve-paddle-ndk28c)
- Host strip-unneeded; NDK 28.2 link-gate LINK_OK all six .so
- INT8 stamps: arm64 jni fp16 calib; armv7 jni fp32 calib; x86 calib on light
- Fixes: ANDROID_NDK paths, Android OpenMP, no -msse3 on ARM, cmake 3.28 pin
- libpin.toml + artifact/ + app jniLibs updated; commit on libpin-paddle-cleanup
- Eng-log worktree still diverged from master (+a); this append only

## 2026-08-04 - paddle 3-ABI: arm tailor + armv7 tailor + x86 slim (NDK r28c)

- arm64-v8a: tailor+fp16 strip-unneeded jni/light ~1.8MB; stamps uint8_to_fp16/fp32_to_uint8; bit-identical to prior pin
- armeabi-v7a: **NEW tailor** (fp32 calib, no arm82 FP16) jni/light **~0.75MB** (was slim ~3.0MB); stamps uint8/int8_to_fp32 + fp32_to_uint8; NDK28 LINK_OK
- x86_64: slim thin-jni ~53KB + light ~10MB (bit-identical prior); android-x86 LITE_BUILD_TAILOR drops KernelRegistrar (missing fp32_to_uint8) — default profile stays slim; no USE_LITE_KERNEL force-refs (touch_* DCE)
- Host strip + link-gate PASS all 3 ABIs; get-artifacts → artifact/ + app jniLibs; libpin.toml + SOURCE.md updated
- Build notes: patch_tailor_conv_copy_safe mounted; x86 no longer auto-tailors when paddle-models present

## 2026-08-04 - armv7 zero-heatmap root cause + fix

- Root cause: optimize_armv7 used --quant_model=true (broken det.nb, all-zero heatmaps).
- Fix: analytic uint8 input only; det output uint8; rec float CTC; no weight quant.
- Runtime: slim armv7 light/jni default; keep-registry (no LTO) for all Android builds.
- Verified: QEMU + Pixel OCR PASS ABCD12345 angle~-14.4°.
...
## 2026-08-05 - Full 16KB packaging + First 10 validation (emu-5554)

- useLegacyPackaging=false; CMake -Wl,-z,max-page-size=16384 + ANDROID_SUPPORT_FLEXIBLE_PAGE_SIZES=ON
- Experiment deep links: vehicleexpenses://experiment/{align,pump}?auto=first10
- Before/after First 10 on emulator-5554: ALIGN full semantic MATCH; PUMP deskew+set labels 10/10 MATCH (line1 cost/vol OCR nondeterminism only)
- APK x86_64/arm64 natives Stored + LOAD Align 0x4000
- Scratch: dev-ai-interaction/scratch/16k-5554-first10-20260805/
- Residual (armv7 purity): OpenCV armv7 pin; paddle armv7 max-page
