...
- In plan mode: you have **zero authority** to edit, create, or modify any *tracked files outside the sandbox* (i.e. the real application source code that lives in git, such as files under `app/`, build scripts, main documentation, etc.). All intended changes to tracked files outside `dev-ai-interaction/` must be fully described in a formal sandbox plan. The user must explicitly approve that plan with the magic phrasing before any execution sub-agent may touch those files.
  - **Explicit exception for TODO.md**: The tracked `TODO.md` at the worktree root **may** be edited by planners (e.g. to record the approved plan as first action, or update the checklist). This is the one tracked file exception so the planner does not have to maintain a separate TODO copy in the sandbox.
- You **are allowed** to work freely inside the sandbox (`dev-ai-interaction/**`) and on local untracked per-worktree files (e.g. `project-facts.md`). The `.grok/config.toml` rules + `plan-mode-hard-stops.js` hook enforce the boundary.
...
- **Planning and Execution Subagent Separation (Recommended for Complex Work — Preferred Architecture):** For non-trivial work, the main (top-level / orchestrator) agent should act primarily as coordinator and reviewer rather than doing the heavy lifting itself. This reduces the pressure on any single thread to "be helpful" by rushing to implementation or calling exit_plan_mode too early.
  ...
  **Recommended flow:**
  1. Main agent receives the request. If in planning, it may immediately spawn a dedicated **Planning Sub-agent** (subagent_type="plan" or "explore", capability_mode="execute" followed by narrow planning prompt) with a *very strict* prompt:
     "You are the Planning Sub-agent. On every launch or after any gap, **first** read the local untracked project-facts.md (or .agent-state/project-facts.md) in the worktree root + the current plan file in dev-ai-interaction/plans/. Treat these as your persistent memory for the planning state. Your *only* responsibilities are research (using read/grep/list_dir + full shell/git/adb investigation) and producing or iteratively revising a high-quality plan document in dev-ai-interaction/plans/ following the standard structure. 
...
The master (via its built-in prompt in `run-grok-master`) now automates this. The human triggers it with a short phrase in the master terminal: "New planning cycle" (optionally + one short sentence of context/feedback). The master reads project-facts.md + the latest `standard-plan-compliance-block.md`, writes a complete narrow prompt file (full dedicated planner template + embedded current guardrails + cycle context) to `dev-ai-interaction/.planning-agent-prompt.txt`, outputs the exact one-line restart instruction to the user, and then stops.
...
In addition to the sandbox plan document, each worktree has its own **local untracked per-branch state file** (e.g. `project-facts.md` or `.agent-state/project-facts.md` directly in the worktree root, not in the shared dev-ai-interaction/ sandbox; these are gitignored). On every fresh launch or new cycle, **first** read the local project-facts.md (this worktree). These local state files provide cheap per-branch continuity for *codebase facts and pointers* without polluting the shared sandbox or git history. Do not store planning data, active plan references, execution steps, or "I'm working on" narrative here — those belong in the designated plan file (dev-ai-interaction/plans/) or ENGINEERING_LOG.md (dated effort entries). TODO.md (the tracked root file) is the exception for checklist/plan recording and may be edited by planners.

**project-facts.md Content and Hygiene Rules (CRITICAL — prevents token exhaustion and re-derivation bloat)**
- **Facts and pointers only about the codebase and branch implementation state.** It is *not* a log of what you are doing, not a plan tracker, not a history of steps or decisions. Never instructions, plan text, "active plan", phases, "plan passed", or process narrative.
  - Keep structure minimal and stable: Branch, current builds tag.
  - Codebase facts and pointers: main source files and their roles for the current experiment/branch, key data structures and sizes in use (e.g. BufferSet dimensions), important functions/locations (takeSnapshot, red box storage, ICRS handling, JSON output), where to find logic for specific behaviors, locations of analysis scripts/reports in dev-ai-interaction/ if they describe current implementation, external paths for test data (photos, reports).
- **Update discipline**:
  - During *planning* (interactive/strategy) or *execution*: append *only* new discovered codebase facts or pointers (1-2 lines). Detailed progress, "I'm working on X this cycle", activity logs, user input, or plan-related items must go to ENGINEERING_LOG.md (append a dated entry). Do not re-derive context or repeat plan content.
- **Mandatory hygiene at the start of every new cycle/launch (in addition to before appends)**: On every fresh launch or new planning cycle (after re-reading the designated plan), perform a full hygiene pass on project-facts.md before doing any other work or appends:
  1. Read the *entire* project-facts.md file using read_file (no offset/limit or tail). Always read in full — a tail will miss critical data.
  2. If the file is large (e.g., many lines or > ~8 KB), report its exact size (use wc -l or equivalent tool) in your output for a separate work item to address bloat. Do not silently prune or roll without explicit directive.
  3. Prune: remove anything that is not a stable codebase fact or pointer. Delete old or transient items.
  4. Remove any plan references, execution steps, narrative, or bloat.
  5. Only keep core facts + a single line note if rolling older content to ENGINEERING_LOG.md + git.
- **Mandatory hygiene before every append (especially in multi-step execution or after compaction)**: Same steps as above (full read in one shot, report size if large), but performed immediately before any write to project-facts.md.
- This keeps re-reads cheap on every cycle/launch and stops the agent from bloating its own *output* tokens. The sandbox plan + git + build tags + narrow tool results + ENGINEERING_LOG.md are the source of truth for current work and history. project-facts is only for quick codebase orientation facts.
...
- You may also edit local untracked per-worktree files such as `project-facts.md`.
...
- Before each `./build_app`: `git add` the changed tracked source file(s) + `TODO.md`. (project-facts.md is deliberately untracked/gitignored and must never be `git add`ed.)

- project-facts.md updates: only new stable codebase facts/pointers (no plan references or execution tracking). Always read full project-facts.md (report size if large). ENGINEERING_LOG.md is append-only (only append new dated entries at end; never edit prior content).
...
The "approved plan" for feature work is always the content of the designated sandbox file the user explicitly named, never the harness session plan.md or any historical archive. Local untracked per-worktree state files (project-facts.md) are the mechanism for branch-specific continuity.
