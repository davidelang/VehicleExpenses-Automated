...
When reading current-state.md: always read the *full* file (no offset/limit or tail). If large, report its size for separate work. When appending to ENGINEERING_LOG.md: *only append* a new dated entry at the end — never edit prior sections.
...
- **Mandatory hygiene at the start of every new cycle/launch (in addition to before appends)**: On every fresh launch or new planning cycle (after re-reading the designated plan), perform a full hygiene pass on current-state.md before doing any other work or appends:
  1. Read the *entire* current-state.md file using read_file (no offset/limit or tail). Always read in full — a tail will miss critical data.
...
- **Mandatory hygiene before every append (especially in multi-step execution or after compaction)**: Same steps as above (full read in one shot, report size if large), but performed immediately before any write to current-state.md.
- This keeps re-reads cheap on every cycle/launch and stops the agent from bloating its own *output* tokens. The sandbox plan + git + build tags + narrow tool results + ENGINEERING_LOG.md are the source of truth for current work and history. current-state is only for quick codebase orientation facts.
...
- current-state.md updates: only new stable codebase facts/pointers (no plan references or execution tracking). Always read full current-state.md (report size if large). ENGINEERING_LOG.md is append-only (only append new dated entries at end; never edit prior content).
