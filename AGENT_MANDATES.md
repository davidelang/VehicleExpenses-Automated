...
When the master wants to begin a new planning cycle it writes a fresh narrow prompt (containing the current strict instructions + the latest standard block) to `dev-ai-interaction/.planning-agent-prompt.txt` and instructs the user: "New planning cycle. Please restart your planner terminal now with the updated prompt file (run `./run-grok-planner` again in that terminal, or `exec ./run-grok-planner` to replace the process in place)." This keeps the planner on the latest instructions without requiring a full relaunch of the master.
...
- **Post-Handoff Cycle Start Protocol and Low-Friction Injection (long-lived terminals):** After any handoff (build + "ready to test" + END marker), the prior execution turn is finished per the Completion and Handoff rule above. In the recommended long-lived two-terminal mode the master writes a fresh narrow prompt for the Planning Agent (including the current standard block and all hygiene rules) and tells the user to restart the planner terminal with the updated prompt file. This is the normal, low-friction way to begin a new planning cycle while keeping both the master and planner terminals open. The planner must treat the next user input after restart (or after being told a new cycle has started) as a brand new planning phase: re-read current-state.md (with required prune), produce a fresh high-signal plan, etc.
  ...
  The master is responsible for generating a fresh narrow prompt at the beginning of each new planning cycle and for clearly instructing the user to restart the planner terminal with it.
