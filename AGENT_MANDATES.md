...
- The harness `~/.grok/sessions/.../plan.md` (process/orchestration log) receives *only* a short entry: "In plan mode. Created project plan at dev-ai-interaction/plans/xxx-plan.md. Summary: [3-5 bullets]. Called exit_plan_mode. Awaiting explicit user approval of the file at that path + directive." 
...
- On handoff or new-cycle start, the just-executed plan document (in dev-ai-interaction/plans/) is moved (or clearly noted) into historical-plans/.
