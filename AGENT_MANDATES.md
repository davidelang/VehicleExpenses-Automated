...
- In plan mode: you have **zero authority** to edit, create, or modify any *tracked files outside the sandbox* (i.e. the real application source code that lives in git, such as files under `app/`, build scripts, main documentation, etc.). All intended changes to tracked files outside `dev-ai-interaction/` must be fully described in a formal sandbox plan. The user must explicitly approve that plan with the magic phrasing before any execution sub-agent may touch those files.
  ...
  - The tracked `TODO.md` at the worktree root **may** be edited (record approved plan, maintain backlog).
  - The tracked `project-facts.md` at the worktree root **may** be edited (add or correct stable location / layout facts).
...
- Call `./build_app` directly (e.g. `./build_app @phase_summary.txt changed.kt TODO.md ...`). Do **not** run manual `git add` / commit. `./build_app` is the single authoritative staging+commit+build+tag gate (it handles the add internally using the supplied files and @message). Confirm success (record the new branch-scoped builds tag).
...
In addition to the sandbox plan document, each worktree root contains `project-facts.md` (tracked). On every fresh launch or new cycle, **first** read the local `project-facts.md`. It holds only stable layout facts and "where things live" pointers that would remain valid for other work on the tree after the current branch is merged and a new effort begins. It must not contain branch names, builds tags, active plan references, "I'm working on", or transient per-experiment details — those belong in the designated plan file or ENGINEERING_LOG.md.

Planners may edit the tracked `project-facts.md` (and `TODO.md`) as explicit exceptions.

**project-facts.md Content and Hygiene Rules (CRITICAL)**
- **Stable facts and locations only.** Content must remain valid after the current branch is merged and a new worktree is created for different work. Never include branch, builds tag, plan names, "working on this cycle", compliance reports, execution status, or experiment-specific constants.
...
- **Mandatory full read + hygiene**:
  ...
  - If the file grows large, report its size for separate cleanup work.
- The sandbox plan + ENGINEERING_LOG.md + git history + narrow tool results are the source of truth for current work. project-facts.md exists to let the planner (and other agents) know immediately where plans, failure logs, instructions, and core scripts live without repeated discovery commands.
...
- **Phased Small-Step Execution must be ultra-micro with per-phase success gates**: The "Phased Small-Step Execution" section **must** decompose the work into as many explicitly named, ultra-small phases as needed to keep each phase the smallest practical observable unit of work that can be forensically verified in isolation and completed with a successful `./build_app`. There is no target number of phases (more smaller phases is better than fewer larger ones, as long as each is a meaningful, verifiable step). Every phase **must terminate with** narrow forensic `read_file` (offset/limit) + targeted grep before/after the edit + a direct call to `./build_app` (e.g. `./build_app @phase_summary.txt changed-file.kt TODO.md ...`). Do **not** perform manual `git add`, `git commit`, or direct gradle/gradlew. `./build_app` is the only correct gate — it does the add + commit (using the files you list and any @message) + build (where applicable) + tag. A confirmed successful build + new branch-scoped builds tag must be recorded **before the next phase's edits may begin**. On trouble, user-directed partial reset, or recovery, only the tag of the most recent successful phase's `./build_app` may be used (after `./get-builds-tag.sh` preflight). This discipline is mandatory for safe, auditable execution turns.
