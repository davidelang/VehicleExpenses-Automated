...
- User approval must be unambiguous and path-specific (see MULTI_AGENT_USER_INSTRUCTIONS.md for the exact magic phrases). In execution you must re-read *exactly* the designated sandbox plan file (fresh) before any source change.
- On handoff or new-cycle start, the just-executed plan document is moved (or clearly noted) into historical-plans/.

The "approved plan" for feature work is always the content of the designated sandbox file the user explicitly named, never the harness session plan.md or any historical archive.
