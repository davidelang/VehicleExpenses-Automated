...
- Only `./run-grok-imagine` pins `--permission-mode always-approve`. Its Landlock is `$SANDBOX/research/yellow-ink-map` + `$HOME/.grok` + tmp — not the full sandbox, not the worktree. Do **not** yolo other `run-grok*`. Do **not** put `allow = ["*"]` on project `.grok/config.toml`.
