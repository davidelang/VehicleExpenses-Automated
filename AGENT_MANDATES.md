...
- Version integrity: commit via `./build_app` before builds that matter for `git describe`. Any commit message that is not a single short `-m` line **must** be `./build_app @file paths…` (write the file first). No quoted multi-line `-m` and no heredoc in the Shell tool. No standalone `git add` / `git commit` on execute. Plan + eng-log + git carry “why”; tags carry state.
...
- Only `./run-grok-imagine` pins `--always-approve` and `--permission-mode bypassPermissions` (the CLI has no `always-approve` mode value). Its Landlock is `$SANDBOX/research/yellow-ink-map` + `$HOME/.grok` + tmp — not the full sandbox, not the worktree. Do **not** yolo other `run-grok*`. Do **not** put `allow = ["*"]` on project `.grok/config.toml`.
