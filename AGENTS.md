...
- In addition, each worktree maintains its own **local untracked per-branch state file** (`current-state.md` or `.agent-state/current-state.md` in the worktree root; gitignored). On cycle start the agent must read the local current-state.md (this worktree) first for cheap codebase facts and pointers (see AGENT_MANDATES for revised rules: facts about files, structures, locations only — no planning data).
