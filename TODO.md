...
- [ ] ultra-micro + gates plan per user 'smaller chunks' + 'failed again': move setup one line/branch at a time into procs; extraction one piece; delete outer one small chunk; multi self-verify gates after first action and before edits; strict only .kt; stop if ritual skipped. Plan: /home/dlang/git/VehicleExpenses-automated/dev-ai-interaction/plans/pump-experiment-complete-proc-transition-excise-outer-special-blocks-20260615-plan.md (2026-06-15 execution start on fix-pump-experiment; post prior resets from 3 failure logs; first source edit performed now per plan + critical first actions; hygiene prune on current-state after this). (First source edit per plan contract + mandates + logs lesson: identical first-action deviation every prior run - must do TODO first then verifs before any kt read/edit. Full phases: Phase 0 hygiene/gates/verify-current-state-first + red box regression fix in C/D/E + baseline build; Phases 1-7 one ultra-tiny hunk/decl/branch/comment per edit + forensic + build + state update; Phase 8 final verify + build. Anti-doom: review 2026-06-15 failure logs first on error, propose 2-3 *different* minimal repairs, apply one new, verify; literal preflight only if needed. Record: prior run on this plan not completed moves/excisions - only partial procA decls + anti-doom dups, outer blocks + flowName vestigial + CDE red regression remain.)
