...
- [ ] Pump Experiment: Refactor to array of per-flow processor functions (user-approved pattern: "an array of functions that you can iterate over is fine") + deliver Set C (pump-only + alignment Set J valley bin-test: 64bin hist, findValleyMidpoints, per-midpoint THRESH_BINARY, independent detect/redbox(+1/nest per scale + cross filter) on each binarized, stacked composite in Set C column via stackVertically, best version for Paddle result+crops). No modifications to pBuild* reporting.
  - **CURRENT TURN (approved plan - process first)**: Strict turn-boundary enforcement to prevent "lone wolf" continuation after handoff. After any `./build_app` success + handoff ("ready to test"), response *must* include exact marker: "**END OF EXECUTION TURN. Awaiting new directive or plan approval before any further source changes or investigation that leads to edits.**" No edits after feedback without new plan in the session plan.md + explicit user "approved, proceed". 
  - Narrow technical goal for this turn (per user feedback after deploy): Make the C processor's output (the composite from doValleyForC containing greyscale first + one labeled binarized image per valley, with text starting "thresh = XX" + reds) the final value used in the Set C PD column in the report. The processor call happens, but old duplicated discovery/path/viz code after the call for Set C can interfere or the composite is not preserved. 
  - Phased small steps (this turn only, per the top section of the session plan.md):
    - Update plan.md (done) + this TODO.md with the process rules + goal (this edit).
    - Forensic read of the exact processor call site + immediately following old body.
    - Small guard: after `flowProcessors[i](...)` when flowName == "Set C", skip/guard the following old `// 3. Discovery` (the inline scales.forEach) and later unconditional path/viz blocks for normal pdHunks (two small boundary replaces: open the guard after the call, close the else before the per-flow } ). This lets the processor composite "win" without large restructure.
    - If labels not visible in the stacked image per user test: tiny tweak to labelWithText (e.g. larger textSize, outline, or header strip) or the calls in doValleyForC. One replace.
    - Add minimal error surfacing (try/catch or log) inside the procC bridge or doValleyForC so silent failures that cause fallback to greyscale are obvious.
    - `./build_app` (list only the kt). Hand-off with the explicit END-OF-TURN marker.
    - Verification: User re-deploys, re-runs pump experiment. In the report, Set C Paddle column shows the multi-image composite (greyscale first + per-valley binarized, each with readable "thresh = XX" label + reds). A/B unchanged. No regression.
  - On any post-handoff feedback in future: immediately treat as new planning cycle (new report + update this plan.md + wait for directive). No lone-wolf continuation.
  - Reuses: doValleyForC, labelWithText, stackVertically, existing guards in the kt, OcrUtils. Do not touch pBuild* or reporting.
  - Locked: current builds tag (no reset past it except allowed checkout . for uncommitted in turn). 3-3-3 applies. Forensic read before/after every replace. Update this TODO first in execution (done).
    ...
    - 2026-06-11: Array of per-set processor functions (listOf A/B/C lambdas) + forEachIndexed dispatch (call by i after the list) is in and builds (tag bc5fb530, from ce9da336). The dispatch iterates the array of functions; the C entry is the dedicated per-path for the valley bin-test (full code in the approved plan and described in the C comment; no hard-coded "Set C" checks inside the per-path code itself -- the array + index is how we select/iterate, per the clarification avoiding ugly hard-coding of names like "setCvalley" at call sites or inside paths). Phase 1 helpers (doCross, param getFinal) in from earlier. Old body temp (tangle for A/B; skeletal C does nothing, old body does work for C -- transition). The valley spec is in the C entry of the array structure. Handoff: the array + structure for the pump experiment (C entry for valley) is ready to test. Later phases (per plan) will fill A/B/C processors with their linear per-path logic (valley in C) and remove the old tangled body. No reset past bc5fb530.
    - Small fix for deployed symptom (broken <img> / no images for Set C in HTML report): the per-flow viz had `if (flowName == "Set C") { // already set to composite inside Set C block } else { set PD from pdHunks }`. With C processor still skeletal (does not set images["PD"]), the if branch for C did nothing → empty images["PD"] → broken data: base64 img in pBuildHtmlRowDynamic for Set C Paddle column. TEMP fix: for Set C viz, force set the PD image using current pdHunksRawTotal (normal discovery for now). Comment explains transition + plan to let processor set composite later (when C entry filled + call after discovery). ML guard for C unchanged. Forensic read before/after. Build succeeded (new tag ac18993b). User: re-deploy the APK, re-run pump experiment; Set C column should now have a valid image (normal red raw PD for now; stacked valley composite when C processor implemented per plan). Path for C Paddle should also be present (old body sets it).
    - 2026-06-11 user report: binarization for Set C not happening (only greyscale seen in report column). Expected: greyscale (deskewed no-stretch) + one binarized image per valley midpoint in the 64-bin hist, each labeled with the threshold used for the split (e.g. "thresh=128"), stacked via stackVertically as the PD image for Set C, with reds from detection on that binarized version; best version's hunks for the result. The C entry in flowProcessors is still placeholder (skeletal), so old body runs normal greyscale path. To activate: hoist runPaddleDiscovery and doCrossScaleRedboxFilter before the flowProcessors list (so names resolve in C lambda); add labelWithText helper (decode bmp, canvas drawText with thresh, re-encode); replace C placeholder with full valley impl (greyscale first labeled, midpoints loop: binarize/swap/clear/runPaddle/doCross/labeled snapshot with reds + thresh text, track best, stack at end, set best pd* and path); wrap the processor call in runBlocking(Dispatchers.IO) to allow suspend calls inside the (suspend) C lambda; update the viz if (flowName == "Set C") branch to not overwrite (preserve processor composite); small edits only, forensic read before/after each, update this TODO, build after pieces, no reset past current builds tag. Success when report shows the expected stacked labeled binarized versions for Set C (plus greyscale), A/B unchanged.
    - Algorithm tweak per user: in doValleyForC (Set C valley path), instead of 64-bin hist on the entire (deskewed) image, first run detection on the grayscale to get the initial raw red boxes, build an OpenCV mask (255 inside those rects only), then calcHist with that mask so the bins (and thus findValleyMidpoints + per-thresh binarizations) are derived only from pixels inside the red boxes (text regions). The binarization is still applied to the full mat for each valley thresh, followed by re-detection, labeling each version snapshot with its thresh, stacking (greyscale first + the binarized ones), best tracking. This focuses the valley thresholds on actual content. Small localized change inside the existing doValleyForC. Guard from prior turn (skip old body for C) still helps the composite win. Error surfacing in bridge remains.
    - Activated: doValleyForC (the dedicated per-path suspend fun with the full valley: greyscale first + per-valley binarized with "thresh = XX" label via labelWithText + reds from detection on binarized, stack for PD composite, best, path) is in, called from the C processor entry (normal procC with runBlocking bridge). The array (listOf procA/procB/procC) + dispatch iterates it, C entry calls the per-path function (no hard-coded name checks inside the valley code). Hoists and helper in. Viz updated to preserve. BUILD SUCCESSFUL (new tag). The binarization now happens for Set C, the report column will have the greyscale + the labeled binarized versions as expected. User: re-deploy the APK, re-run the experiment, inspect the Set C PD image in the HTML (stacked with labels starting with the thresh). A/B unchanged. This delivers the decomposed plan for Set C in the array structure per the clarification. Later phases (plan) for full A/B fill and old body removal if desired. No reset past the tag.
