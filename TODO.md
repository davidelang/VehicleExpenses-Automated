...
- [ ] Execution of pump-experiment-cleanup-per-red-histogram-generation-c-e-20260615-plan.md (VERY FIRST SOURCE EDIT this TODO after re-read current-state.md (hygiene prune first per rules) + FULL plan read (abs) + all 6 failure logs read + record lessons on identical first-action skips; compliance re-read of STANDARD BLOCK first 10 lines + last 5 current-state + git --porcelain no .kt + grep TODO pre (no match); Phase 0: narrow forensic reads of per-red capture blocks (remnant/procC/procE), generateHistogramB64/takeSnapshot/perRedHtml/runPumpExperiment/BufferSet createCrop + BUFFER_SET_SPEC.md + targeted grep per-red C/E for perMask/Mat.zeros/generate/redboxHistC_* ; update TODO first (this) + verifs + baseline git add + ./build_app; then ultra-micro Phase1 capture switch (createCrop+direct calcHist+dual takeSnapshot from longLived; release only rect crop; remove perMask/zeros/rect/generate; comment updates); Phase2 dual visuals in report builder; Phase3 long-lived hist BufferSet init at pump start; Phase4 (global if in scope); all gates/forensic/grep before/after every edit, git add .kt+state+TODO before each build; scope strictly per-red C/E histogram gen cleanup only (no pdHunk etc); 3-3-3 + anti-doom + literal preflight only; end with exact END marker + results ready (tag) + plan path)
