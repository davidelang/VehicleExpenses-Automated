...
- [ ] ultra-micro plan per user smaller chunks: move setup one decl/branch at a time into procs; extraction one piece; delete outer one small chunk; gates + verify TODO/state after first action and before edits; strict only .kt : /home/dlang/git/VehicleExpenses-automated/dev-ai-interaction/plans/pump-experiment-complete-proc-transition-excise-outer-special-blocks-20260615-plan.md . 2026-06-15 execution start (post prior reset; branch fix-pump-experiment). (First source edit per plan contract + mandates. Full phases: Phase 0 hygiene/gates/forensic site reads + baseline build; Phases 1-7 one ultra-tiny hunk/decl/branch/comment per edit + forensic + build + state update; Phase 8 final verify + build. Anti-doom: review 2026-06-15 failure logs first on error, propose 2-3 different minimal repairs, one new change, verify; literal preflight reset if needed.)
