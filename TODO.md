...
- [ ] remove-dead-addnew-and-normalizedtoicrs-plan.md — Remove Dead Code - AddNewVehicleScreen and normalizedToIcrs (approved plan at /home/dlang/git/VehicleExpenses-automated/dev-ai-interaction/plans/remove-dead-addnew-and-normalizedtoicrs-plan.md)
  ...
  - [x] Phase 5: Final verification (full grep app/src zero for both terms; re-read IcrsMath.kt confirms normalized func gone + remaining funcs intact; re-read ManageVehiclesScreen key sections confirm only pixelToIcrs/icrsToPixel + ICRS-always comments no 0-1; full project grep no lingering in kt/java (only TODO/plan docs); current-state updated; git add TODO + build)
...
- [x] finish-normalized-removal-leftover-cleanup-20260617-plan.md — Finish Normalized Removal — Leftover Diagnostic Calc + Stale Doc Cleanup (approved plan at /home/dlang/git/VehicleExpenses-automated/dev-ai-interaction/plans/finish-normalized-removal-leftover-cleanup-20260617-plan.md)
  - [x] Phase 1: Hygiene reads (current state + prior artifacts) (read current-state.md, the prior plan file (for context only), this plan, relevant TODO section, and narrow sections of the two critical source files (AlignmentScreen refinement block + API.md); confirm via grep that debugCropDir / custom Rect classes / rawDiscovery* are absent from app/src/*.kt; log key facts e.g. "leftover block still at 483", "docs still lists PhotoAlignmentUtils"); done
  - [x] Phase 2: Add this plan's ultra-micro checklist to TODO.md (insert new top-level entry for this plan with all checkboxes starting unchecked; narrow forensic read_file (offset around insertion site) + grep for unique phase title before/after; git add TODO.md; ./build_app (success + record tag in current-state.md 1-line)); done
  - [x] Phase 3: Excise the dead diagnostic crop calc block + comment (AlignmentScreen.kt) (perform single smallest edit: delete the comment and the 14 lines of l/t/r/b ... p2 calc (lines ~483-496); leave surrounding if (globalWinnerRef.vehicle.id >= 0) { ... } intact; use unique old_string; narrow forensic read_file (offset ~470 limit 50) + targeted grep for "Diagnostic High-Quality Crop" and "val l = globalWinnerRef.vehicle.odometerCropLeft" before + after; git add the .kt + TODO.md; ./build_app + record new tag); done
  - [x] Phase 4: Verify the Alignment edit (narrow read confirming the dead calc is gone but refinement loop + later calls remain; grep the file for any remaining "Diagnostic|Save to disk using native snapshot" or the orphaned p1/p2 after the resize (must be 0); git add + ./build_app); done
  - [x] Phase 5: Update docs/reference/API.md (remove stale Photo reference) (edit the section header from ### `ImageAlignmentUtils.kt`, `PhotoAlignmentUtils.kt`, & `NativeImageUtils.kt` to ### `ImageAlignmentUtils.kt` & `NativeImageUtils.kt`; remove the bullet `- `PhotoAlignmentUtils` (Object): Utilities for aligning camera photos with reference images.`; keep other bullets; narrow forensic read_file (offset ~40 limit 20) + grep for "PhotoAlignmentUtils" in the docs file before/after; git add + ./build_app); done
  - [x] Phase 6: Project-wide final hygiene grep (run full grep for any remaining references to the removed items in the original scope (debugCropDir, the four custom classes, rawDiscovery*, and the specific dead comment); also grep for "PhotoAlignmentUtils" (must only hit historical TODO or this plan if we reference it); record that src/ is clean; git add TODO.md (if changed) + ./build_app); done
  - [x] Phase 7: Key site forensic reads + final build (narrow `read_file` on the exact edit sites in AlignmentScreen (post-edit refinement if) and API.md; confirm extractFromPhoto, OcrEngine, etc. remain clean (quick targeted greps); git add tracked changes + TODO.md; ./build_app (capture final tag)); done
  - [x] Phase 8: State file updates + completion marker (update current-state.md (1-2 lines only): new active plan pointer + "Leftover cleanup complete. Final tag: XXX. Results ready."; mark the phases in TODO.md as done with minimal notes; output in the execution log the exact string: "**END OF EXECUTION TURN. Awaiting new directive or plan approval before any further source changes or investigation that leads to edits.**" followed by "results ready to test (new tag: XXX)"); done
- [ ] finalize-normalized-cleanup-handoff-marker-todo-hygiene-20260617-plan.md — Finalize Normalized Cleanup Handoff — Ultra-Micro Process, TODO Hygiene, and Exact END Marker (approved plan at /home/dlang/git/VehicleExpenses-automated/dev-ai-interaction/plans/finalize-normalized-cleanup-handoff-marker-todo-hygiene-20260617-plan.md)
  - [ ] Phase 1: Full hygiene re-reads (current state) (read current-state.md (prune hygiene), prior finish plan (context), this plan, relevant TODO (finish-normalized), narrow kt refinement ~470-520 + API ~40-60; greps confirm no debugCropDir/old classes/rawDiscovery*/"Diagnostic High-Quality Crop" in app/src/*.kt (hist only in plans/TODO); git status --porcelain, git log -5, ./get-builds-tag.sh; record obs)
  - [ ] Phase 2: Insert this final plan's ultra-micro checklist into TODO.md (add entry under active with phases starting unchecked; narrow forensic read_file (insert site) + grep unique title before/after; git add TODO.md; ./build_app; record new tag + 1-line in current-state.md)
  - [ ] Phase 3: Forensic re-verify of AlignmentScreen edit site (no edit) (narrow read_file offset 470 limit 60; targeted grep removed strings ("Diagnostic High-Quality Crop", `val l = ...odometerCropLeft`) expect 0; confirm live odoAnns ICRS/iterative intact; git add TODO.md; ./build_app + tag record)
  - [ ] Phase 4: Forensic re-verify of API.md (no edit) (narrow read offset 40 limit 25; grep "PhotoAlignmentUtils" expect 0/historical; git add TODO + ./build_app)
  - [ ] Phase 5: Project-wide hygiene grep + confirm clean state (full grep app/src and docs for remnants + plan terms; record "src/ clean" fact; git add TODO.md; ./build_app)
  - [ ] Phase 6: Key site final forensic reads + prepare TODO (narrow reads of edit sites (Alignment post-ref if, API header) + current-state + TODO entry; greps extractFromPhoto / OcrEngine clean; git add tracked + TODO.md; ./build_app)
  - [ ] Phase 7: Final build gate with full hygiene (re-run ./get-builds-tag.sh preflight; git add TODO; ./build_app record tag; update current-state.md exactly 1-2 lines)
  - [ ] Phase 8: Output exact END marker + handoff (output exact marker string in trace; mark only this plan's phases minimally; git add TODO.md; final ./build_app)
