...
- Phase 1: forensic reads of per-flow body (~274+), preproc C/E, remnant; grep closed-over (pd*/ml/ scales/ t*/ flowName/ buffers/ funs); narrow read procA stub. Mechanical: procA replaced with full dupe logic + exact `val flowName = "Set A"` at top + param aliases (workspace=ws etc). Forensic read/grep after for "procA" / "Set A". No if(flowName) edits yet.
