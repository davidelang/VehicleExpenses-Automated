...
static cv::Mat planeRoi8u(cv::Mat* plane, int x, int y, int w, int h) {
    ...
    cv::Mat r = (*plane)(cv::Rect(x, y, w, h));
    r.setTo(0);
    return r;
}
...
static void seg7One(
    ...
    *usedFb = 1;
    ...
    cv::Mat lookBin;
    if (overlayY && scratchFits(overlayY, imgW, imgH)) {
        lookBin = (*overlayY)(cv::Range(nt, nb), cv::Range(sl, sr));
        lookBin.setTo(0);
    } else if (scratchFits(scratch, sr - sl, nb - nt)) {
        lookBin = planeRoi8u(scratch, 0, 0, sr - sl, nb - nt);
    }
    const int localT = st - nt;
    const int localB = sb - nt;
    PoisonStats stLocal;
    cv::Mat* objPlane = (inkDump && scratchFits(inkDump, imgW, imgH)) ? inkDump : nullptr;
    const int sPx = fillPoisonLookRaster(
        seedY, look, localT, 0, srcIsBin, gm, fallback, &lookBin,
        overlayY, overlayUv, sl, nt, poisonStats ? &stLocal : nullptr, scratch,
    ...
    *usedFb = (sPx == fallback) ? 1 : 0;
    ...
    auto hasBar = [&](int y) {
        if (objPack && objPlane) {
            return rowHasInkId(*objPlane, nt + y, sl, sr, objPack->inkLo, minRun, glareW);
        }
...
static int fillPoisonLookRaster(
    ...
    if (objPack && objPlane && scratchFits(objPlane, objPlane->cols, objPlane->rows)) {
        uint8_t minInk = 255;
        uint8_t maxNon = 0;
        bool anyInk = false, anyNon = false;
        for (int y = 0; y < objPlane->rows; ++y) {
            const uint8_t* p = objPlane->ptr<uint8_t>(y);
            for (int x = 0; x < objPlane->cols; ++x) {
                const uint8_t v = p[x];
                if (!v) continue;
                if (v >= 128) {
                    anyInk = true;
                    ...
                    anyNon = true;
                    if (v > maxNon) maxNon = v;
                }
            }
        }
        if (anyInk) {
            objPack->inkLo = minInk;
            objPack->nextInk = static_cast<uint8_t>(minInk > 0 ? minInk - 1 : 0);
        }
        if (anyNon) objPack->nextNon = static_cast<uint8_t>(maxNon < 255 ? maxNon + 1 : 255);
    }
    ...
    if (srcIsBin) {
        ...
        if (haveClean) {
            ...
            sample = planeRoi8u(scratch, 0, seedH, seedW, seedH);
            if (sample.empty()) {
                sample.create(seedH, seedW, CV_8UC1);
                sample.setTo(0);
            }
        }
    }
    ...
    if (!srcIsBin && inverted && haveClean && !cleanDark) {
        ...
        if (nFirst > 0 &&
            ...
            cv::Mat keep2Local;
            cv::Mat keep2 = planeRoi8u(scratch, 0, 0, seedW, seedH);
            if (keep2.empty()) {
                keep2Local.create(seedH, seedW, CV_8UC1);
                keep2 = keep2Local;
            }
            for (int yy = 0; yy < seedH; ++yy) {
            ...
            float frac2 = 0.f;
            cv::Mat sample2Local;
            cv::Mat sample2 = planeRoi8u(
                (overlayY && (!scratch || overlayY->data != scratch->data)) ? overlayY : nullptr,
                0, 0, seedW, seedH);
            if (sample2.empty()) sample2 = planeRoi8u(scratch, 0, seedH, seedW, seedH);
            if (sample2.empty()) {
                sample2Local.create(seedH, seedW, CV_8UC1);
                sample2 = sample2Local;
            }
    ...
    cv::Mat combinedLocal;
    cv::Mat combined = planeRoi8u(scratch, 0, seedH * 2, seedW, seedH);
    if (combined.empty()) {
        combinedLocal.create(seedH, seedW, CV_8UC1);
        combinedLocal.setTo(0);
        combined = combinedLocal;
    }
    ...
    if (objPack && objPlane) {
        ...
        if (objAlloc(objPack, true, seedIndex, kKindInk, &inkId)) {
            ...
            for (int yy = 0; yy < seedH; ++yy) {
                ...
                for (int xx = 0; xx < seedW; ++xx) {
                    if (cp[xx]) objPut(objPlane, ovX + xSeed0 + xx, ovY + ySeed0 + yy, inkId);
                }
            }
        }
        stampPoisonFlood(poison, objPlane, ovX + xSeed0, ovY + ySeed0, sPx, seedIndex, objPack);
    }
    cv::Mat labels, stats, centroids;
    ...
    for (int i = 1; i < nLab; ++i) {
        ...
        for (int yy = r.y0; yy < r.y1; ++yy) {
            ...
            uint8_t* kp = keepR.ptr<uint8_t>(yy - r.y0);
            for (int xx = r.x0; xx < r.x1; ++xx) {
                if (lp[xx] == i) { kp[xx - r.x0] = 255; ++nR; }
            }
        }
        ...
        if (srcIsBin) {
            ...
            for (int yy = r.y0; yy < r.y1; ++yy) {
                const uint8_t* bp = bin.ptr<uint8_t>(yy);
                const uint8_t* kp = keepR.ptr<uint8_t>(yy - r.y0);
                uint8_t* rp = rBin.ptr<uint8_t>(yy - r.y0);
                for (int xx = r.x0; xx < r.x1; ++xx) {
                    if (kp[xx - r.x0] && bp[xx]) rp[xx - r.x0] = 255;
                }
            }
            fillSaltPepper(&rBin);
            ...
            bool rDark = true;
            float rFrac = 0.f;
            cv::Mat seedRoi = seedY(cv::Range(r.y0, r.y1), cv::Range(r.x0, r.x1));
            if (!otsuKeep(seedRoi, keepR, &rThr, &rDark, &rFrac)) {
                r.noPeak = true;
                regs[static_cast<size_t>(i)] = r;
                continue;
            }
            applyThrKeep(seedRoi, keepR, rThr, rDark, &rBin);
            fillSaltPepper(&rBin);
            HorizSW hhR = horizPeakSW(rBin, seedH, seedW);
            r.noPeak = hhR.peak <= 4;
            r.thr = rThr;
        ...
        if (!r.noPeak) {
            cv::Mat combRoi = combined(cv::Range(r.y0, r.y1), cv::Range(r.x0, r.x1));
            orBin(&combRoi, rBin);
        }
        regs[static_cast<size_t>(i)] = r;
    }
    ...
    cv::Mat lookPoisonLocal;
    cv::Mat lookPoison;
    if (scratchFits(scratch, lw, lh) &&
        !(overlayY && scratch && overlayY->data == scratch->data)) {
        lookPoison = (*scratch)(cv::Rect(0, 0, lw, lh));
        lookPoison.setTo(0);
    } else {
        lookPoisonLocal.create(lh, lw, CV_8UC1);
        lookPoisonLocal.setTo(0);
        lookPoison = lookPoisonLocal;
    }
    for (int yy = 0; yy < seedH; ++yy) {
...
static jintArray aabbGrayMany(
    ...
    for (int i = 0; i < n; ++i) {
        ...
        seg7One(*gray, l, t, r, b, imgW, imgH, &ol, &ot, &orr, &ob, &sPx, &vSW, &hSW, &fb,
            ...
            &poisonPacks[static_cast<size_t>(i)], scratch, &objPack, i);
        if (objPack.abort) {
...
static jintArray aabbColorMany(
    ...
    for (int i = 0; i < n; ++i) {
        ...
        cv::Mat* tintDst = scratchFits(scratch, imgW, imgH) ? scratch : nullptr;
        cv::Mat* poisonScratch = (tintDst == scratch) ? overlayY : scratch;
        objPack.classChange = 0;
        objPack.seenInk = 0;
        objPack.seenNon = 0;
        ...
        if (ok && tintDst && !tintDst->empty() && !skipTintWalk(true, tele)) {
            seg7One(*tintDst, l, t, r, b, imgW, imgH,
                ...
                &sweeps[static_cast<size_t>(i)], inkDump, overlayY, overlayUv,
                &poisonPacks[static_cast<size_t>(i)], poisonScratch, &objPack, i);
...
static void seg7OrientedOne(
    ...
    *sPxOut = static_cast<float>(fallback);
    ...
    const int sPx = fillPoisonLookRaster(
        ...
        overlayY, overlayUv, 0, 0, poisonStats ? &stLocal : nullptr, scratch,
        nullptr, nullptr, 0,
...
static jfloatArray seg7OrientedMany(
    ...
    for (int i = 0; i < n; ++i) {
        ...
        float sPx = 2.f;
        cv::Mat* poisonScratch = (src == scratch || cMag == scratch) ? overlayY : scratch;
        seg7OrientedOne(*src, box, imgW, imgH, op, &sPx,
            boundStrategy, tightInsetPx, srcIsBin, &tele, keepColor,
            &sweeps[static_cast<size_t>(i)], inkDump, overlayY, overlayUv,
            &poisonPacks[static_cast<size_t>(i)], poisonScratch);
        op[8] = sPx;
        storeTeleArr(env, teleArr, i, tele);
    }
    writeSweepArr(env, sweepArr, sweeps);
...
Java_com_davidlang_vehicleexpensesautomated_ui_util_NativeImageUtils_nativeGrayOrientTight(
    ...
    return seg7OrientedMany(
        env, grayPtr, uvPtr, scratchPtr, seeds, 0, 0, 16,
        teleArr, sweepArr, dumpPtr, overlayYPtr, overlayUvPtr, poisonArr);
}
...
Java_com_davidlang_vehicleexpensesautomated_ui_util_NativeImageUtils_nativeGrayOrientRetract(
    ...
    return seg7OrientedMany(
        env, grayPtr, uvPtr, scratchPtr, seedsArr, 0, 2, 16,
        teleArr, sweepArr, dumpPtr, overlayYPtr, overlayUvPtr, poisonArr);
}
