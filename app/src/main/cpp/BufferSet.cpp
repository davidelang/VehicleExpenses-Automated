...
Java_com_davidlang_vehicleexpensesautomated_ui_util_BufferSet_nativeCompressYuvToBase64(
    ...
    // Create a compact BGR Mat for encoding
    cv::Mat bgr(h, w, CV_8UC3);
    
    // Manual merge of Y and interleaved UV into BGR
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            uint8_t Y = yData[y * stride + x];
            // NV21: V is at vData, U is at vData + 1 (if pixelStride=2)
            // But we have separate U and V buffers from the handle.
            // Since our internal buffer is NV21, uData = vData + 1.
