...
object ImageAlignmentUtils {
    ...
    // Production cleaning — Experiment 4 text-only mask (working version)
    suspend fun createCleanedReference(original: Bitmap): Bitmap? = withContext(Dispatchers.IO) {
        ...
        // Use OCR on the original bitmap (no temp file needed)
        ...
        // Bitwise AND — keep only text areas
        val origMat = Mat()
    ...
    // alignImages kept for future use
