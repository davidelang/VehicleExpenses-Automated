        ...
        if (detInterpreter != null) {
            ...
            for (cropRectF in dbRes.suspectCrops) {
                ...
                if (cropWidth > 0 && cropHeight > 0) {
                    ...
                    val subInputBuffer = ByteBuffer.allocateDirect(1 * inputSize * inputSize * 3 * 4).order(ByteOrder.nativeOrder())
                    for (y in 0 until inputSize) {
                        for (x in 0 until inputSize) {
                            val px = subPadded.getPixel(x, y)
                            subInputBuffer.putFloat(((px shr 16 and 0xFF) / 255.0f - 0.485f) / 0.229f)
                            subInputBuffer.putFloat(((px shr 8 and 0xFF) / 255.0f - 0.456f) / 0.224f)
                            subInputBuffer.putFloat(((px and 0xFF) / 255.0f - 0.406f) / 0.225f)
                        }
                    }
                    ...
                    // Map sub-boxes back to global coordinates
