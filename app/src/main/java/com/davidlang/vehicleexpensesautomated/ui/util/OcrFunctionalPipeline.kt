...
object OcrFunctionalPipeline {
    ...
    suspend fun runOnBitmap(context: Context, input: Bitmap): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // 6) Letterbox crop into 320×48 rec buffer (pump asis path)
            val tRec0 = System.currentTimeMillis()
            val pW = crop.width()
            val pH = crop.height()
            val sCropId = master.createCrop(crop.left, crop.top, pW, pH)
            val scale = 48f / pH
            val rawW = (pW * scale).toInt()
            val targetW = ((rawW + 31) / 32 * 32).coerceAtMost(320).coerceAtLeast(32)
            val targetH = 48
            recBuffer.p.clear()
            ...
            Imgproc.resize(
                master.c[sCropId].mat,
                recBuffer.c[recCropId].mat,
                org.opencv.core.Size(targetW.toDouble(), targetH.toDouble()),
                0.0,
                0.0,
                interp,
            )
            val ocr = engine.recognize(recBuffer.c[recCropId])
            recBuffer.c[recCropId].release()
            master.c[sCropId].release()
            ...
            Result(
                ...
                debug = mapOf(
                    ...
                    "target_rec" to "${targetW}x$targetH",
