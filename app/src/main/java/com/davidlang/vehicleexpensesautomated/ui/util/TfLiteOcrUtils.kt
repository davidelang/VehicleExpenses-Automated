...
object TfLiteOcrUtils {
    ...
    fun decodeCtcGreedy(logits: Array<Array<FloatArray>>, dictionary: List<String>, blankIndex: Int = 0): Pair<String, Float> {
        ...
        for (t in 0 until timeSteps) {
            ...
            if (maxIndex != blankIndex && maxIndex != lastIndex) {
                ...
                if (maxIndex >= 0 && maxIndex < dictionary.size) { 
                    result.append(dictionary[maxIndex])
                    totalConf += maxVal
                    count++ 
                }
            }
    ...
    private fun calculateAdaptiveThreshold(contours: List<MatOfPoint>): Float {
        ...
        for (b in 0..maxBucket) {
            if ((bins[b] ?: 0) == 0 && (bins[b+1] ?: 0) == 0) {
                if (aspects.count { it < b * 0.5 } >= (aspects.size * 0.5)) { 
                    gapBucketStart = b
                    break 
                }
            }
    ...
    private fun processSubBlob(rect: Any, invScale: Double, sourceW: Double, sourceH: Double, sourceMat: Mat?, algorithm: String, rawBoxes: MutableList<DetectedBox>, refinedBoxes: MutableList<DetectedBox>) {
        ...
        val redFloorPixels = android.graphics.Rect(
            (rawBounds.left * sourceW).toInt(),
            (rawBounds.top * sourceH).toInt(),
            (rawBounds.right * sourceW).toInt(),
            (rawBounds.bottom * sourceH).toInt()
        )
        val sourceRect = RotatedRect(
            Point(rotatedRect.center.x * invScale, rotatedRect.center.y * invScale),
            Size(rotatedRect.size.width * invScale, rotatedRect.size.height * invScale),
            rotatedRect.angle
        )
        ...
        val normRefinedBounds = RectF(
            (bounds.left.toFloat() / sourceW.toFloat()).coerceIn(0f, 1f),
            (bounds.top.toFloat() / sourceH.toFloat()).coerceIn(0f, 1f),
            (bounds.right.toFloat() / sourceW.toFloat()).coerceIn(0f, 1f),
            (bounds.bottom.toFloat() / sourceH.toFloat()).coerceIn(0f, 1f)
        )
    ...
    private fun expandInRoi(rect: RotatedRect, sourceMat: Mat, algorithm: String, redFloor: android.graphics.Rect): android.graphics.Rect {
        ...
        val gray = Mat()
        if (roiMat.channels() > 1) Imgproc.cvtColor(roiMat, gray, Imgproc.COLOR_RGBA2GRAY) else roiMat.copyTo(gray)
        val localRedFloor = android.graphics.Rect(redFloor.left - roiL, redFloor.top - roiT, redFloor.right - roiL, redFloor.bottom - roiT)
        val expandedRect = expandByValleyStop(localRedFloor, gray)
        gray.release()
        roiMat.release()
    ...
    private fun expandByValleyStop(redFloor: android.graphics.Rect, gray: Mat): android.graphics.Rect {
        var minX = redFloor.left.toDouble()
        var maxX = redFloor.right.toDouble()
        var minY = redFloor.top.toDouble()
        var maxY = redFloor.bottom.toDouble()
        ...
        val maxH = gray.rows()
        val maxW = gray.cols()
        val hL = (maxX - minX) * 4.0
        val vL = (maxY - minY) * 1.0
        val sX = minX
        val sXX = maxX
        val sY = minY
        val sYY = maxY
        ...
        while (minY > 0 && (sY - minY) < vL) { 
            if (getLineInkCount(gray, minX.toInt(), maxX.toInt(), (minY - 1).toInt(), true, valleyThreshold) < 8) break
            minY -= 1.0 
        }
        while (maxY < maxH - 1 && (maxY - sYY) < vL) { 
            if (getLineInkCount(gray, minX.toInt(), maxX.toInt(), (maxY + 1).toInt(), true, valleyThreshold) < 8) break
            maxY += 1.0 
        }
        while (minX > 0 && (sX - minX) < hL) { 
            if (getLineInkCount(gray, minY.toInt(), maxY.toInt(), (minX - 1).toInt(), false, valleyThreshold) < requiredBridgeHeight) break
            minX -= 1.0 
        }
        while (maxX < maxW - 1 && (maxX - sXX) < hL) { 
            if (getLineInkCount(gray, minY.toInt(), maxY.toInt(), (maxX + 1).toInt(), false, valleyThreshold) < requiredBridgeHeight) break
            maxX += 1.0 
        }
    ...
    private fun getLineInkCount(mat: Mat, start: Int, end: Int, fixed: Int, horizontal: Boolean, threshold: Double): Int {
        var inkPixels = 0
        val maxD = if (horizontal) mat.cols() else mat.rows()
        if (fixed < 0 || fixed >= (if (horizontal) mat.rows() else mat.cols())) return 0
        for (i in start until end) { 
            if (i < 0 || i >= maxD) continue
            val pixelValue = if (horizontal) mat.get(fixed, i)[0] else mat.get(i, fixed)[0]
            if (pixelValue > threshold) inkPixels++ 
        }
        return inkPixels
    }
        ...
        while (changed) {
            ...
            while (i < mutableRefined.size) {
                ...
                while (j < mutableRefined.size) {
                    ...
                    val interL = max(boxA.left, boxB.left)
                    val interT = max(boxA.top, boxB.top)
                    val interR = min(boxA.right, boxB.right)
                    val interB = min(boxA.bottom, boxB.bottom)
