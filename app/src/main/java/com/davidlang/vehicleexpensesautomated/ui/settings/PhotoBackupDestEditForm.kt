...
@Composable
internal fun PhotoDestEditForm(
    destId: String,
    totalDestCount: Int,
                    ...
                    scope.launch {
                        ...
                        try {
                            ...
                            if (result.success) {
                                ...
                                consentRecovery.launch(result.recoveryIntent) { runTest(allowRecovery = false) }
                            } else {
            ...
            onSyncNow = if (isNew) {
            ...
            } else {
                {
                    scope.launch {
                        ...
                        syncInProgress = true
                        statusIsError = false
                        statusText = "Starting backup for this destination…"
                        try {
                            val result = withContext(Dispatchers.IO) {
                                viewModel.syncNow(
                                    accountHint = accountHint,
                            ...
                            Toast.makeText(
                                context,
                                if (result.success) "Backup complete" else (
...
@Composable
internal fun S3ProviderDropdown(
    selected: S3ProviderPreset,
