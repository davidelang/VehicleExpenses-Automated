...
### Staged modes (preferred for SO A/B)

Do **not** reimplement the full app pump tree here. Split the paddle surface so you can see *which* step diverges:

| `--stage` | Checks | App analogue |
|-----------|--------|----------------|
...
```bash
# Stage A/B two SOs (heatmap + boxes + optional rec crop):
./scripts/paddle-ocr-stage-ab.sh \
  --so-a path/to/pin/libpaddle_light_api_shared.so \
  --so-b path/to/cand/libpaddle_light_api_shared.so \
  --image fixtures/or/L1.pgm \
  --rec-image path/to/digit_crop.pgm \
  --sides "224 608 1024" --resize topleft --threads 1

# Single stage:
./third_party/paddle/tests/ocr_functional/qemu/out/x86_64/paddle_ocr_functional \
  --abi x86_64 --stage det-heat --det … --rec … --dict … --image L1.pgm \
  --det-side 1024 --resize topleft --threads 1
```

**Resize modes:** `topleft` ≈ app `prepareScale` long-edge + `populateMonoUInt8` top-left pad into square.  
`letterbox` = center pad (legacy `pipeline` / skewed_hello).

**Still not the app:** multi-scale red-box filter, blue/orange crops, cost/vol classify, OpenCV minAreaRect, DNG decode, threads races under concurrent work. First 10 remains the product gate until a stage can fail known-bad SOs.

### Pipeline mode (default gate)
